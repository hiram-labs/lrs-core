...
const whitelist = [
  {
    searchPath: ['statement', 'authority'],
    filter: ['objectType', 'member', 'mbox', 'account', 'mbox_sha1sum', 'openid'],
    displayKey: 'name'
  },
  {
    searchPath: ['statement', 'actor'],
    filter: ['objectType', 'member', 'mbox', 'account', 'mbox_sha1sum', 'openid'],
    displayKey: 'name'
  },
  ...
  {
    searchPath: ['statement', 'context', 'instructor'],
    filter: ['objectType', 'member', 'mbox', 'account', 'mbox_sha1sum', 'openid'],
    displayKey: 'name'
  },
  {
    searchPath: ['statement', 'context', 'team'],
    filter: ['objectType', 'member', 'mbox', 'account', 'mbox_sha1sum', 'openid'],
    displayKey: 'name'
  },
  ...
  {
    searchPath: ['statement', 'context', 'contextActivities', 'category', '*'],
    filter: ['id'],
    displayKey: 'definition.name'
  },
  { searchPath: ['statement', 'context', 'contextActivities', 'category', '[]', 'definition', 'type'] },
  {
    searchPath: ['statement', 'context', 'contextActivities', 'grouping', '*'],
    filter: ['id'],
    displayKey: 'definition.name'
  },
  { searchPath: ['statement', 'context', 'contextActivities', 'grouping', '[]', 'definition', 'type'] },
  {
    searchPath: ['statement', 'context', 'contextActivities', 'parent', '*'],
    filter: ['id'],
    displayKey: 'definition.name'
  },
  { searchPath: ['statement', 'context', 'contextActivities', 'parent', '[]', 'definition', 'type'] },
  {
    searchPath: ['statement', 'context', 'contextActivities', 'other', '*'],
    filter: ['id'],
    displayKey: 'definition.name'
  },
  { searchPath: ['statement', 'context', 'contextActivities', 'other', '[]', 'definition', 'type'] },
...
export const getAtPath = (object, { searchPath, filter, displayKey }, currentPath = []) => {
  ...
  if (key === '[]' && Array.isArray(object)) {
    return object.map((item) => getAtPath(item, { searchPath: remainingSearchPath, filter, displayKey }, currentPath));
  }

  return getAtPath(object[key], { searchPath: remainingSearchPath, filter, displayKey }, [...currentPath, key]);
};

export default function (statement) {
  const statementData = statement instanceof Statement ? statement.toObject() : statement;

  const flatMap = flatten(map(whitelist, (pathData) => getAtPath(statementData, pathData)));

  const withoutMissingValues = reject(flatMap, (flatPath) => isUndefined(flatPath.path) || isUndefined(flatPath.value));
