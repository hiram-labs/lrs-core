...
describe('addIdentsToCache', () => {
  beforeEach((done) => {
    Promise.all([QueryBuilderCache.deleteMany({}), QueryBuilderCacheValue.deleteMany({})]).then(() => done(), done);
  });
  ...
  it('should udapte the query builder cache for one ident', (done) => {
    addIdentsToCache(organisation, [uniqueIdentifier, { key, value }])
      .then(() =>
        Promise.all([
          assertCacheKeys(1, {
            organisation,
  ...
  it('should not update the query builder cache for no idents', (done) => {
    addIdentsToCache(organisation, [uniqueIdentifier])
      .then(() => Promise.all([assertCacheKeys(0, {}), assertCacheValues(0, {})]))
      .then(
        () => done(),
        (err) => done(err)
      );
