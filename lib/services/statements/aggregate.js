...
import getDashboardFromAuthInfo from 'lib/services/auth/authInfoSelectors/getDashboardFromAuthInfo';
import getShareableFromAuthInfo from 'lib/services/auth/authInfoSelectors/getShareableFromAuthInfo';
...
export const dashboardPipeline = async ({ pipeline, authInfo }) => {
  ...
  const shareable = dashboard && find(dashboard.shareable, (share) => share._id.toString() === shareableId);
...
export default async ({ authInfo, limit, skip, cache, maxTimeMS, pipeline, sampleSize }) => {
  ...
  return new Promise((resolve, reject) =>
    Statement.aggregateByAuth(authInfoWithOrgId, pipelineWithDashboard, options, (err, results) =>
