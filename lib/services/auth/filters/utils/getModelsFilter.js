...
export const getSiteAdminFilter =
  () =>
  ({ authInfo }) => {
...
export const checkAllowedTokenType =
  ({ allowedTokenTypes = ['organisation', 'client'] }) =>
  ({ authInfo }) => {
    const tokenType = getTokenTypeFromAuthInfo(authInfo);
    if (!includes(allowedTokenTypes, tokenType)) {
      throw new NoAccessError();
    }
  };

export const checkAllScope =
  () =>
  ({ authInfo }) => {
    const scopes = getScopesFromAuthInfo(authInfo);
...
export const checkOrganisationExpirery =
  () =>
  async ({ authInfo }) => {
    ...
    if (org && org.expiration && moment(org.expiration).utc().isBefore(moment().utc())) {
      throw new NoAccessError();
    }
  };
