...
describe('model scope filters organisation', () => {
  ...
  testOrgScopeFilter(modelName, 'view', [MANAGE_ALL_ORGANISATIONS], TEST_USER_ORGS_FILTER);
  ...
  it('should return the correct view filter when using a owner token', async () => {
    ...
    const actualScopeFilter = await getModelScopeFilter(modelName, 'view', token, user);
  ...
  testClientBasicScopeFilterError(modelName, 'create', [ALL]);
