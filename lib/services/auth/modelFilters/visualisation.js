...
import { shareableModelFilter } from 'lib/services/auth/filters/getShareableModelFilter';
  ...
  async ({ actionName, authInfo }) => {
    ...
    if (tokenType === 'dashboard') {
      ...
      const dashboards = await Dashboard.aggregate([
        { $match: { _id: new ObjectId(dashboardId) } },
        { $unwind: '$widgets' },
        {
          $project: {
            _id: 0,
      ...
      return { _id: { $in: map(dashboards, (item) => new ObjectId(item.visualisationId)) } };
    }
  };

const filters = [getSiteAdminFilter, checkAllowedTokenType, getDashboardFilter, checkAllScope, shareableModelFilter];
