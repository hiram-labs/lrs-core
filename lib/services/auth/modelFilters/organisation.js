...
const getUserViewableOrgs = async (authInfo, scopes) => {
  ...
  } else if (tokenType === 'organisation' || tokenType === 'user') {
    organisationIds = get(authInfo, ['user', 'organisations'], []);
  }
  return organisationIds;
};

const getUserEditableOrgs = async (authInfo, scopes, validOrganisationTokenScopes = [ALL]) => {
  let organisationIds = [];
  const tokenType = getTokenTypeFromAuthInfo(authInfo);
  if (tokenType === 'client') {
    if (includes(scopes, ALL)) {
    ...
    case 'organisation':
      {
        ...
        if (intersection(scopes, allowedCreateScopes).length === 0) {
...
const createFilterQuery = (organisationIds) => ({ _id: { $in: organisationIds } });
...
export const runScopeFilterWithOrg = (args) => async (orgId) => {
  ...
  if (!includes(stringOrgIds, orgId)) {
    throw new NoAccessError();
  }
};
