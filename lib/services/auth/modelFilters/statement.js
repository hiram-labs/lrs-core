...
import getFilterFromAuthInfo from 'lib/services/auth/authInfoSelectors/getFilterFromAuthInfo';
...
const readAllScopes = [scopes.ALL, scopes.XAPI_ALL, scopes.XAPI_READ, scopes.XAPI_STATEMENTS_READ];

const hasReadMineScope = (tokenScopes) => {
  const hasReadAllScope = tokenScopes.filter((scope) => readAllScopes.includes(scope)).length !== 0;
  return !hasReadAllScope && tokenScopes.includes(scopes.XAPI_STATEMENTS_READ_MINE);
};
...
const hasWriteScope = (tokenScopes) => {
  ...
  return includes(tokenScopes, scopes.XAPI_ALL) || includes(tokenScopes, scopes.XAPI_STATEMENTS_WRITE);
};
...
export default async ({ actionName, authInfo, allowDashboardAccess = false }) => {
  ...
  if ((actionName === 'create' || actionName === 'edit') && !hasWriteScope(tokenScopes)) {
