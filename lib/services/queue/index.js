...
const getProvider = ({ queueProvider = process.env.QUEUE_PROVIDER }, done) => {
  switch (queueProvider) {
    ...
    case 'LOCAL':
      return done(null, localProvider);
    default:
      return done(new Error(`${queueProvider} is not a valid queue provider`));
  }
};
...
export const publish = ({ queueName, payload }, done = defaultCallback) => {
...
export const subscribe = ({ queueName, handler, onProcessed }, done = defaultCallback) =>
