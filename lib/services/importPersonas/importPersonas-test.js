...
describe('import personas service', () => {
  ...
  it('should successfully import a persona', async () => {
    ...
    const structure = {
      testName: { columnType: COLUMN_NAME },
      mbox: { columnType: COLUMN_MBOX, primary: 1 }
    };

    const csvHandle = `${PERSONAS_CSV_PATH}/${id}.csv`;
    await uploadFromStream(csvHandle)(
    ...
    const onProcessedPromise = new Promise((resolve) => {
      importPersonasWorker({
        onProcessed: async () => {
          const processedPersonaImport = await PersonasImport.findOne({ _id: id });
    ...
    // # TEST
    const { importPersonasPromise } = await importPersonas({ id, personaService, authInfo });
    await importPersonasPromise;

    // Wait for the queues to process
    await onProcessedPromise;

    const personas = await connection.collection('personas').find({}).toArray();
  ...
  describe('addErrorsToCsv', () => {
    it('Should add errors to the csv', async () => {
      ...
      const csvPromise = highland(downloadCsvStream)
        .map((data) => data)
        .collect()
        .toPromise(Promise);

      await downloadToStream(csvHandle)(downloadCsvStream);
