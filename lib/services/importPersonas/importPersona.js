...
import { getPersonaName, getIfis, getAttributes } from 'lib/services/importPersonas/personasImportHelpers';
...
export default ({ personaImportId, structure, organisation, personaService }) =>
  async (row, rowIndex) => {
    ...
    if (erroringIfis.length > 0 || ifis.length === 0) {
      const errors = erroringIfis.map((ifi) => validateIfi(ifi, ['ifi']));
        ...
        { _id: personaImportId },
        {
          ...
          $push: {
            importErrors: {
              row: rowIndex,
              rowErrors:
                ifis.length === 0 ? 'No ifis' : flattenedErrors.map((err) => `${err.path.join('.')}: ${err.data}`)
            }
          }
        }
      );
    ...
    try {
      ...
      const personaIdentifiers = await map(ifis, (ifi) =>
        personaService.createUpdateIdentifierPersona({
          organisation,
          personaName,
          ifi
        })
