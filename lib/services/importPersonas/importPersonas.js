...
import { STAGE_PROCESSING, PERSONA_IMPORT_QUEUE } from 'lib/constants/personasImport';
...
export const addErrorsToCsv = async ({ csvHandle, csvOutHandle, personasImport }) => {
  ...
  let i = 1;
  const highlandStream = highland(csvStream).flatMap((data) => {
    const out = highland(
      (async (dat) => {
        let ou;
...
const processDataThroughWorker =
  ({ personaImportId, structure, organisation }) =>
  (data, i) => {
...
const importPersonasLong = async ({ id, personaImport, personaService }) => {
  const { structure, csvHandle, organisation } = personaImport;
...
const importPersonasShort = async ({ id, authInfo }) => {
...
const importPersonas = async ({ id, personaService, authInfo }) => {
