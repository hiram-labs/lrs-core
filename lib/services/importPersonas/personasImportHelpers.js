...
import { includes, filter, get, mapValues, chain, isNull } from 'lodash';
...
export const getPossibleRelatedColumns = ({
  ...
  const relatedStructures = filter(
    structureWithName,
    (column) => relatedType !== undefined && relatedType === column.columnType
  );
...
export const resetRelatedStructure = ({
  ...
  structure.map((item, key) => {
    if (item.get('relatedColumn') === columnName || key === columnName) {
      let newItem = item;
...
export const updateRelatedStructure = ({
  ...
  if (relatedColumn) {
    const out = structureWithRelatedColumn
...
export const isColumnOrderable = ({ columnStructure }) => {
  if (!columnStructure) {
    return false;
  }
...
export const getIfis = ({
  ...
  const ifis = ifiKeyColumns
    .map(({ columnName, columnType, relatedColumn, useConstant = false, constant }) => {
      if (columnType === COLUMN_IFI_KEY) {
        if (
          // ignore un set rows in the csv
          !row[columnName] ||
          !row[relatedColumn]
        ) {
          return null;
...
export const getAttributes = ({
  ...
  const attributes = attributeColumns
    .map(({ columnName, relatedColumn, attributeName }) => {
...
export const getPrimaryMaxPlusOne = ({
  ...
  const max = structure.max((item) => item.get('primary'));
