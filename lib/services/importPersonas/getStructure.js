...
export const generateStructure = ({ csvHeaders, structure }) => {
  ...
  const out = allStructureMap.sortBy(
    (value, key) => key,
    (keyA, keyB) => {
...
const getMatchingStructures = ({ csvHeaders, filter }) =>
  PersonasImport.aggregate([
    {
      $match: {
        csvHeaders: {
          $not: {
            // csvHeaders is a super set.
            $elemMatch: {
              $nin: csvHeaders
            }
          }
...
export default async ({ csvHeaders, filter }) => {
