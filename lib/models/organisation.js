...
import { VIEW_PUBLIC_DASHBOARDS, VIEW_PUBLIC_VISUALISATIONS, VIEW_PUBLIC_JOURNEYS } from 'lib/constants/orgScopes';
 ...
 */
schema.statics.limitIdsByOrg = function limitIdsByOrg(stepIds, cumulativeIds, level = 0, cb) {
  if (level > 0) {
    Organisation.find({ parent: { $in: stepIds } }).then((orgs) => {
      const childIds = map(orgs, '_id');
      Organisation.limitIdsByOrg(childIds, union(childIds, cumulativeIds), level - 1, cb);
    });
  } else {
    cb(cumulativeIds);
  }
...
const createRoles = async (orgModel) => {
  ...
  await Role.create({
    ...
    scopes: [VIEW_PUBLIC_DASHBOARDS, VIEW_PUBLIC_VISUALISATIONS, VIEW_PUBLIC_JOURNEYS]
  });
...
const updateOwner = async (orgModel, User, adminRole) => {
  ...
  owner.organisationSettings = union(owner.organisationSettings || [], [
    {
      organisation: orgModel._id,
      roles: [adminRole._id]
    }
...
schema.post('deleteOne', { document: true, query: false }, async (orgModel, next) => {
