...
export const getUTCDateTime = (UTCHours, UTCMinutes, dateToAddTimeTo) => {
  ...
  const utcDateTime = moment(dateToAddTimeTo).utc();
...
export const getWindowDates = (windowStartUTCHour, windowStartUTCMinute, windowDurationSeconds) => {
  const todaysWindowStartTime = getUTCDateTime(windowStartUTCHour, windowStartUTCMinute, moment().utc());
  const todaysWindowEndTime = moment(todaysWindowStartTime).utc().add(windowDurationSeconds, 'seconds');
...
export const inWindow = (siteSettings) => {
  ...
  if (windowStartUTCHour && windowStartUTCMinute && windowDurationSeconds) {
    // Extract the time and work out the value for today
    const { todaysWindowStartTime, todaysWindowEndTime } = getWindowDates(
      windowStartUTCHour,
      windowStartUTCMinute,
      windowDurationSeconds
    );
    // check if the current time is between the start time and the end time
    const insideWindow = moment()
      .utc()
      .isBetween(moment(todaysWindowStartTime).utc(), moment(todaysWindowEndTime).utc());
    return insideWindow;
  }
...
export const nextRunAtDateTime = (siteSettings) => {
  ...
  const nextRunAtDate = moment().utc().isSameOrAfter(todaysWindowStartTime)
...
const schema = new mongoose.Schema({
  ...
  pageSize: {
    // No of documents to delete at a time.
    type: Number,
    default: 1000
  },
  total: {
