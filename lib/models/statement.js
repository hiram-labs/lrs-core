...
const ASYNC_AGGREGATION_REFRESH_AFTER_SECONDS = defaultTo(
  Number(process.env.ASYNC_AGGREGATION_REFRESH_AFTER_SECONDS),
  60
); // 1 minute
...
schema.writeScopes = keys(scopes.USER_SCOPES).concat([scopes.XAPI_ALL, scopes.XAPI_STATEMENTS_WRITE]);

schema.plugin(scopeChecks);
schema.plugin(filterByOrg);
schema.plugin(addCRUDFunctions);

schema.post('deleteOne', { document: true, query: false }, async (statement, next) => {
...
const streamAggregation = ({ pipeline, skip, limit, batchSize, maxTimeMS }) => {
  ...
  return Promise.resolve(query.cursor({ batchSize }));
};
 ...
 * filtered by the lrs visible to the authenticated client
...
schema.statics.aggregateByAuth = function aggregateByAuth(
  ...
  return parseQuery(pipeline, { authInfo })
    .then(async (parsedPipeline) => {
      ...
      if (cache === false) {
        return streamAggregation({
          ...
          .then((stream) => streamToStringResult(stream))
          .then((result) => {
            cb(null, result);
          });
      }
      ...
      const redisKey = cachePrefix(
        `${organisation}-AGGREGATION-${sha1(JSON.stringify(finalPipeline))}-${skip}-${limit}`
      );
        ...
        .then((dataKeyTTL) => {
          ...
          if (dataKeyTTL <= AGGREGATION_REFRESH_AT_SECONDS) {
            ...
            streamPromise = streamAggregation({
              ...
              limit,
              batchSize
            }).then((stream) => setCachedAggregation({ client: redisClient, dataKey, isRunningKey, stream }));
          }
 ...
 * @returns {void}
 */
export const runAggregationAsync = async (redisClient, organisationId, pipeline, skip, limit) => {
  ...
  await redisClient.set(`${prefix}-STARTED-AT`, moment().utc().format('YYYY-MM-DD HH:mm:ss.SSSZ'));
  ...
  try {
    ...
    const completedAt = moment().utc().format('YYYY-MM-DD HH:mm:ss.SSSZ');
...
export const getAggregationStatus = async (redisClient, prefix, sinceAt) => {
  ...
  if (sinceAt && completedAt && moment(completedAt).utc().isSameOrBefore(moment(sinceAt).utc())) {
    return {
      result: null,
      startedAt,
      completedAt: null,
...
const hasFreshCache = (completedAt) => {
  ...
  const cacheExpiryTime = moment(completedAt).utc().add(ASYNC_AGGREGATION_REFRESH_AFTER_SECONDS, 'seconds');
  return moment().utc().isSameOrBefore(cacheExpiryTime);
};
 ...
 * @returns {Promise}
 */
export const aggregateAsync = async (authInfo, basePipeline, skip, limit, sinceAt) => {
  const redisClient = redis.createClient();
