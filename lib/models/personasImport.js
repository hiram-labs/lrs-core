...
import { keys, chain, map, includes, isUndefined, isInteger, isNull, get, uniq } from 'lodash';
...
/**
 * @typedef {object} PersonaImportField
 *  @property {string} [columnName]
 *  @property {string} [columnType]
 *  @property {string} [relatedColumn]
 *  @property {number} [primary]
 *  @property {string} [attributeName]
 *  @property {boolean} [useConstant]
 *  @property {string} [constant]
 */

/** @typedef {Object<string, PersonaImportField>} PersonasImportStructure */

/**
 * @typedef {object} PersonasImportError
 *  @property {number} row
 *  @property {string[]} rowErrors
 */

/**
 * @typedef {object} PersonasImportResult
 ...
 * @typedef {object} PersonasImport
 *  @property {string} title
 *  @property {*} owner TODO: define type
 *  @property {*} organisation TODO: define type
 *  @property {string} csvHandle
 *  @property {string} csvErrorHandle
 *  @property {string} importStage TODO: add enum
 *  @property {number} totalCount
 *  @property {number} processedCount
 *  @property {string[]} csvHeaders
 *  @property {Date} importedAt
 *  @property {PersonasImportError[]} importErrors
 *  @property {PersonasImportResult} result
 *  @property {PersonasImportStructure} structure
 */

/** @typedef {module:mongoose.Model<PersonasImport>} PersonasImportModel */
  ...
  const out = chain(structure)
    .map(
      ...
      (value, key) => {
        ...
        if (get(value, 'useConstant', false) && !get(value, 'constant', '').match(/^http(s?):\/\//)) {
...
export const validateAttributeName = (structure) => {
  const attributeNames = Object.values(structure)
...
export const validatePrimaryNumber = (structure) => {
  const invalidStructure = chain(structure)
    .filter((item) => !(isUndefined(item.primary) || isNull(item.primary) || isInteger(item.primary)))
    .value();
...
const schema = new mongoose.Schema({
  ...
  importedAt: {
    type: Date,
    default: moment(DATE_INFINITY).utc().toDate()
  },
  importErrors: {
    type: [
      {
