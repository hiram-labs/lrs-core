...
async function validatePassword(value) {
  ...
  const shouldTestHistoryCheck =
    this.ownerOrganisationSettings.PASSWORD_HISTORY_CHECK && this.ownerOrganisationSettings.PASSWORD_HISTORY_TOTAL > 0;
  ...
  if (shouldTestHistoryCheck) {
    ...
    const sortedHistory = _.sortBy(this.passwordHistory, 'date').slice(
      -1 * this.ownerOrganisationSettings.PASSWORD_HISTORY_TOTAL
    );
  ...
  email: {
    ...
    validate: {
      validator: async function emailValidator(value) {
        const found = await User.countDocuments({ email: value, _id: { $ne: this._id } }); // eslint-disable-line no-use-before-define
        if (found) {
...
schema.methods.createResetToken = function createResetToken(next, expires = 24) {
  bcrypt.hash(this.email, 8, (err, token) => {
    ...
    if (expires && expires > 0) {
      expiresDate = moment().utc().add({ hours: expires }).toDate();
    }
  ...
  if (user.isNew) {
    ...
    User.findOne({ email: user.email }).then((existingUser) => {
      if (existingUser) {
        ...
        existingUser.organisations = _.unionBy(user.organisations, existingUser.organisations, _.toString);
        ...
        const newSettings = _.map(existingUser.organisations, (org) => {
          ...
          const exists = _.find(existingSettings, (setting) => setting.organisation.toString() === orgId);
          if (exists) {
            return exists;
          }
  ...
  if (!preventPasswordHash && user.isModified('password')) {
    bcrypt.hash(user.password, 8, (err, hash) => {
      ...
      if (filteredHistory.length > user.ownerOrganisationSettings.PASSWORD_HISTORY_TOTAL) {
        ...
        user.passwordHistory = _.slice(
          sortedHistory,
          filteredHistory.length - user.ownerOrganisationSettings.PASSWORD_HISTORY_TOTAL
        );
      } else {
...
schema.pre('validate', { document: true, query: false }, function handlePreSave(next) {
  const user = this;
  checkNewUser(user, (checkedUser) => {
    ...
    if (user.isNew) {
      Organisation.findOne({ _id: user.ownerOrganisation })
        .then((org) => {
