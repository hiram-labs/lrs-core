...
export const MAX_TIME_MS = process.env.MAX_TIME_MS ? Number(process.env.MAX_TIME_MS) : 0;
...
const getParsedScopedFilter = async ({ authInfo, filter, modelName, actionName }) => {
  ...
  const filters = [parsedFilter, scopedFilter].filter((item) => !_.isUndefined(item) && !_.isEmpty(item));

  const parsedScopeFilter = filters.length <= 1 ? _.first(filters) : { $and: filters };
  return parsedScopeFilter;
};

const getHasNextPages = ({ models, limit, direction, cursor }) => {
...
const getHasPreviousPages = ({ models, limit, direction, cursor }) => {
...
async function getCount({ filter, authInfo, maxTimeMS = MAX_TIME_MS, hint }) {
  const parsedScopeFilter = await getParsedScopedFilter({
    authInfo,
    filter,
...
async function getById({ _id, authInfo }) {
  ...
  const query = this.find(parsedScopeFilter).limit(1);
  ...
  return query.exec().then((models) => postFetchMap.bind(this)(_.first(models)));
}
