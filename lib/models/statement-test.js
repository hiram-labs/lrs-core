...
import Statement, { getAggregationStatus, runAggregationAsync, buildRedisKeyPrefix } from './statement';
...
describe('models/statement', () => {
  ...
  describe('runAggregationAsync', () => {
    it('should set status and results to redis', async () => {
      ...
      expect(moment(completedAt).utc().isSameOrBefore(after)).to.equal(true);
      expect(moment(completedAt).utc().isSameOrAfter(before)).to.equal(true);
