...
describe('organisation model', () => {
  ...
  it('should properly create organisation', async () => {
    ...
    const createdUsersOrganisation = userAfterOrganisationCreated.organisations.find(
      (id) => id.toString() === organisation._id.toString()
    );
    const createdUsersOrganisationSetting = userAfterOrganisationCreated.organisationSettings.find(
      (orgSetting) => orgSetting.organisation.toString() === organisation._id.toString()
    );
  ...
  it('should properly delete organisation', async () => {
    ...
    const deletedUsersOrganisation = userAfterOrganisationDeleted.organisations.find(
      (id) => id.toString() === organisation._id.toString()
    );
    const deletedUsersOrganisationSetting = userAfterOrganisationDeleted.organisationSettings.find(
      (orgSetting) => orgSetting.organisation.toString() === organisation._id.toString()
    );
    const deletedUsersRoles = await Role.find({ organisation: organisation._id });
  ...
  it('should throw a ValidationError when settings.LOCKOUT_ATTEMPS is 0', async () => {
    ...
    await expect(f).to.eventually.be.rejectedWith(
      Error.ValidationError,
      'A user should be allowed at least one attempt'
    );
  });
