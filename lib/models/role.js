...
export const canDelete = async (role) => {
  ...
  const found = await getConnection().model('Role').countDocuments({
    _id: { $ne: role._id },
...
schema.pre('deleteOne', { document: true, query: false }, async function handlePreRemove(next) {
  const role = this;

  if (!(await canDelete(role))) {
    // This is the last role, so don't delete
    next(new Error('Can not delete last admin role'));
    return;
  }

  if (await hasUser(role)) {
    // A user is assigned to this role, don't delete
