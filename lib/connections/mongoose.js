...
const getConnection = (namespace = 'll') => {
  ...
  connections[namespace].on('connected', () => handleConnected(namespace, `Mongo connection instance ${namespace} connected successfully.`));
  connections[namespace].on('disconnecting', () => handleError(namespace, `Mongo connection instance ${namespace} is disconnecting.`));
  connections[namespace].on('disconnected', () => handleError(namespace, `Mongo connection instance ${namespace} disconnected.`));
