...
import getScopeFilterFromReq from 'lib/services/auth/requestSelectors/getScopeFilterFromReq';
import getScopeSelectFromReq from 'lib/services/auth/requestSelectors/getScopeSelectFromReq';
 ...
 */
export const contextFilter = async function contextFilter(model, req, cb = () => {}) {
...
export const RESTIFY_DEFAULTS = {
  ...
  preMiddleware: (req, res, next) => {
    passport.authenticate(['jwt', 'clientBasic'], DEFAULT_PASSPORT_OPTIONS, (err, user) => {
      ...
      req.user = user;
      return Promise.all([getScopeFilterFromReq(req), getScopeSelectFromReq(req)])
  ...
  onError: (err, req, res) => {
    ...
    } else if (statusMap[err.message]) {
      res.status(statusMap[err.message]);
    }
    res.send(err);
  }
};
