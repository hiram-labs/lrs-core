import { map, has, get, isArray, isPlainObject, isString, identity, mapValues } from 'lodash';
...
const convertMatch = (match) => {
  ...
  parsedMatch = createDateParser('statement.stored', 'stored', convertToDate)(parsedMatch);
  return parsedMatch;
};
const convertSort = (sort) => {
  let parsedSort;
  parsedSort = createDateParser('statement.timestamp', 'timestamp', identity)(sort);
  parsedSort = createDateParser('statement.stored', 'stored', identity)(parsedSort);
  return parsedSort;
};

const convertPipeline = (pipeline) =>
  pipeline.map((stage) => {
    if (has(stage, '$match')) {
      return {
        ...stage,
