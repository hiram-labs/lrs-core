...
/*
  ...
  to the old value.
*/
export const cursorPosition = ({ oldCursorPosition, oldValue, newValue, acc = 0 }) => {
  ...
  if (head(newValue) !== undefined && head(newValue).match(/\s/)) {
    return cursorPosition({
      oldCursorPosition,
      oldValue,
      newValue: tail(newValue),
  ...
  if (head(oldValue) !== undefined && head(oldValue).match(/\s/)) {
    return cursorPosition({
      oldCursorPosition: oldCursorPosition - 1,
      oldValue: tail(oldValue),
      newValue,
