...
describe('filter$lookup helper test', () => {
  ...
  it('should add an organisations pipeline to an allowed collection with no existing pipeline', async () => {
    ...
    const result = await filter$lookup([...firstStages, ...lookupStage, ...lastStages], authInfo);
    const expectedResult = [...firstStages, ...expectedLookupStage, ...lastStages];
  ...
  it('should filter out a $lookup when not in an allowed collection', async () => {
    ...
    const result = await filter$lookup([...firstStages, ...lookupStage, ...lastStages], authInfo);
    const expectedResult = [...firstStages, ...[], ...lastStages];
