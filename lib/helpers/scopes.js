...
import { chain, mapValues, first, includes, without } from 'lodash';
...
const getWriteDependencies = (scope, scopes) => {
  ...
  if (first(lastKey) === 'edit' && includes(scopes, depScope)) {
...
const getAllDependencies = (scope, scopes) => {
  ...
  if (first(key) === 'all' && includes(scopes, depScope)) {
    return [depScope];
  }
  return [];
};
...
export const getScopeDependencies =
  ...
  (scope) => {
    ...
    const subDependencies = chain(dependencies).map(getScopeDependencies(scopes)).flatten().union().value();

    return [...dependencies, ...subDependencies];
  };

export const getScopeDependents =
  (scopes = orgScopes) =>
  (scope) => {
    const dependents = chain(getScopesDependencies(scopes))
      .pickBy((dependencieScopes) => includes(dependencieScopes, scope))
      .keys();
    ...
    const dependentsDepentents = chain(dependents).map(getScopeDependents(scopes)).flatten().union().value();
