 ...
 * @return {any}
 */
const convertOpValue = async (identifier, { authInfo } = {}) => {
  ...
  return {
    $in: without(
      map(attributes, ({ personaId }) => personaId),
      undefined,
      null
    )
  };
...
const convert$personaIdent = (value, options) => {
  ...
  if (_.has(value, op)) {
    ...
    return Promise.all(
      _.map(value, (child, key) =>
        convert$personaIdent(child, options).then((convertedValue) => ({ [key]: convertedValue }))
      )
    ).then((parts) => {
