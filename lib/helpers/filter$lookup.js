import { isPlainObject, reject, isArray, get, set, includes, has, map } from 'lodash';
...
const shouldRemove = (pipeline) =>
  has(pipeline, ['$lookup', 'from']) && !includes(ALLOWED_COLLECTIONS, get(pipeline, ['$lookup', 'from']));
...
const filterPipeline = async (stage, authInfo) => {
  ...
  const authInfoFilterStage = [{ $match: scopedFilter }];

  const existingPipeline = get(stage, ['$lookup', 'pipeline'], []);

  const newPipeline = [...authInfoFilterStage, ...existingPipeline];
