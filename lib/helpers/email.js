...
export const defaultMailOptions = {
  from:
    `${defaultTo(process.env.LL_FROM_NAME, 'skillspacexr')} ` +
    `<${defaultTo(process.env.LL_FROM_EMAIL, 'noreply@v360energy.com')}>`
};
...
export const sendNewUser = (user, token) => {
  const subject = 'You have been invited into an organisation in skillspacexr';
...
export const sendOrganisationAboutToExpire = async (user, organisation) => {
  const subject = `The ${organisation.name} organisation is going to expire on ${moment(organisation.expiration).utc().format()} on skillspacexr`;
  ...
  const content = compiledContent({
    ...
    expiration: moment(organisation.expiration).utc().format()
  });
  const htmlEmail = compiledWrapper({ title: subject, body: content, siteUrl: process.env.SITE_URL });
  const text = htmlToText.fromString(content);
...
export const sendOrganisationExpired = async (user, organisation) => {
  const subject = `The ${organisation.name} organisation has expired on skillspacexr`;
