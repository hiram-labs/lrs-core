...
export const defaultMailOptions = {
  from:
    `${defaultTo(process.env.LL_FROM_NAME, 'Learning Locker')} ` +
...
export const sendOrganisationAboutToExpire = async (user, organisation) => {
  const subject = `The ${organisation.name} organisation is going to expire on ${moment(organisation.expiration).utc().format()} on Learning Locker`;
