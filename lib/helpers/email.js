...
export const defaultMailOptions = {
  from:
    `${defaultTo(process.env.LL_FROM_NAME, 'XRTEMIS')} ` +
...
export const sendNewUser = (user, token) => {
  const subject = 'You have been invited into an organisation in XRTEMIS';
...
export const sendOrganisationAboutToExpire = async (user, organisation) => {
  const subject = `The ${organisation.name} organisation is going to expire on ${moment(organisation.expiration).utc().format()} on XRTEMIS`;
...
export const sendOrganisationExpired = async (user, organisation) => {
  const subject = `The ${organisation.name} organisation has expired on XRTEMIS`;
