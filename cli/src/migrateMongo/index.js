...
const getOutstandingMigrations = async ({ down, up, migrations }) => {
  ...
  const outstandingMigrations = lastKey ? migrations.skipUntil((item, key) => key === lastKey).slice(1) : migrations;
...
export const fixDbMigrationOrder = async ({ migrations }) => {
  ...
  for (const migrationItem of migrations.keySeq().toJS()) {
    ...
    const dbMigration = find(dbMigrations, (dbMigrationItem) => migrationItem === dbMigrationItem.key);
    if (!dbMigration) {
      break;
    }
...
const displayInfo = async ({ info, migrations }) => {
  ...
  if (outstandingMigrations.size > 0) {
    const outstandingMigrationsString = outstandingMigrations.keySeq().toJS().join('\n\t\t');
...
export default async function (
  { down, up, info, migrations = v2Migrations, dontExit = false, fixOrder = false },
  next = null
) {
  try {
    if (info) {
