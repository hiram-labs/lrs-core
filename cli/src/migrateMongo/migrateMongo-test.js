...
describe('migrateMongo', () => {
  ...
  describe('up', () => {
    ...
    it('should error up next if no up migrations left', async () => {
      ...
      await expect(resultPromise).to.eventually.be.rejectedWith(Error, 'No outstanding migrations');
    });
    ...
    it('should error if up key does not exist', async () => {
      ...
      await expect(resultPromise).to.eventually.be.rejectedWith(
        Error,
        'Target migration key does not exist in outstanding migrations'
      );
    });
  });
  ...
  describe('down', () => {
    ...
    it('should error if down not existing target migration', async () => {
      ...
      await expect(result).to.eventually.be.rejectedWith(Error, 'Down key (002-test) not found in the database');
    });
    ...
    it('should error if down is last but there are no run migrations', async () => {
      ...
      await expect(result).to.eventually.be.rejectedWith(Error, 'No migrations have been run, ergo down is invalid');
    });
  });
