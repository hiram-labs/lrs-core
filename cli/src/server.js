 ...
 *
 * @param {(() => Promise)|((options: Command) => Promise)} asyncFunc
 */
...
program.command('clearAggregationCache').option('-o, --org [org]', 'Organisation ID').action(clearAggregationCache);
...
program.command('testStatementForwarding [statementId]').option('-p, --port [port]').action(testStatementForwarding);
...
program.command('disableRegister').action(disableRegister);
...
program.command('orgUsageTracker').action(orgUsageTracker);
