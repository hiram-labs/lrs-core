...
export default function (lrsId, options) {
  ...
  const statementStream = highland(
    Statement.find(query).select({ _id: 1, completedQueues: 1, processingQueues: 1 }).cursor()
  );
  ...
  const queueAddStream = statementStream.batch(batchSize).flatMap((statements) => {
    const promises = map(
      statements,
      (statement) =>
