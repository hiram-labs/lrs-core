...
export default async (statementId) => {
  ...
  statement.completedQueues = filter(statement.completedQueues, (item) => item !== STATEMENT_QUERYBUILDERCACHE_QUEUE);
  statement.processingQueues = filter(statement.completedQueues, (item) => item !== STATEMENT_QUERYBUILDERCACHE_QUEUE);
