...
const createNewIdent = (doc) => {
  ...
  const newValue =
    isPlainObject(value) && has(value, 'homePage') && has(value, 'name')
...
const updateStatementsForFailedIdent = async (failedIdent) => {
  const existingIdent = await newIdentsCollection.findOne({
  ...
  if (existingIdent) {
    console.log(
      new Date(),
      `Convert personaIdentifier ${failedIdent._id} to ${existingIdent._id} with persona name of ${personaDisplay} (${existingIdent.persona})`
    );
    const filter = {
      organisation: new ObjectId(failedIdent.organisation),
...
const migrateIdentifierBatch = (docs) => {
  const opsPromises = [createAttributesFromIdents(docs), insertIdents(docs)];
