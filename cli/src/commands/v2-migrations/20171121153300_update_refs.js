...
import { getActivitiesFromStatement, getRelatedActivitiesFromStatement } from '../queriables/getActivitiesFromStatement';
...
const createIndex = (key) =>
  Statement.collection.createIndex(
    {
      organisation: 1,
      lrs_id: 1,
      [key]: 1
...
const up = async () => {
  const query = {
    $or: [{ rev: { $exists: false } }, { rev: { $lte: REVISION } }]
  };
  ...
  createIndexes(['activities', 'agents', 'registrations', 'relatedActivities', 'relatedAgents', 'verbs']);
};
