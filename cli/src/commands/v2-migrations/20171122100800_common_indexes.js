...
const createStatementIndexes = (connection) => {
  ...
  const createIndexWithOrg = (name, index) => createIndex(stmts, `org_${name}`, { organisation: 1, ...index });
  const createIndexWithStore = (name, index) => createIndexWithOrg(`lrs_${name}`, { lrs_id: 1, ...index });
  const createIndexWithVoid = (name, index) => createIndexWithStore(`voided_${name}`, { voided: 1, ...index });
