...
const updateUser = (user, next) => {
  ...
  const newOrgSettings = _.map(user.organisations, (orgId) => {
    ...
    const existingOS = _.find(
      existingOrgSettings,
      (eos) => eos.organisation && eos.organisation.toString() === orgId.toString()
    );
...
export default function (done = null) {
  ...
  stream.on('data', onStreamData).on('error', onStreamErr).on('close', onStreamClose.bind(null, done));
}
