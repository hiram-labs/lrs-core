...
export default function (statementId, options = {}) {
  ...
  return Statement.findById(statementId)
    .then((statement) => {
      // Remove completed statements.
      // update doc
      statement.completedQueues = filter(statement.completedQueues, (item) => item !== STATEMENT_FORWARDING_QUEUE);

      statement.processingQueues = filter(statement.processingQueues, (item) => item !== STATEMENT_FORWARDING_QUEUE);
