  ...
  async.each(
    ...
    (updateCollection, done) => {
      updateCollection.updateMany(
        ...
        { $set: { createdAt: isodate, updatedAt: isodate } },
        {},
        (err, results) => {
          if (err) throw new Error(err);
          if (results.nModified === 0) {
            logger.info(
              `All documents have timestamps in the ${loggerNames[updateCollections.indexOf(updateCollection)]} collection.`
            );
          } else {
            logger.info(
              `Added timestamps to ${results.nModified} records in the ${loggerNames[updateCollections.indexOf(updateCollection)]} collection.`
            );
          }
          done();
        }
