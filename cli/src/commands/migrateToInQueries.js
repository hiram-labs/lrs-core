...
const isActor = (criteriaPath) =>
  criteriaPath.length === 2 && criteriaPath[0] === 'statement' && criteriaPath[1] === 'actor';
...
const isPersonaId = (criteriaPath) =>
  criteriaPath.length > 2 && criteriaPath[0] === 'persona' && criteriaPath[1] === 'import';
...
const _buildNewQuery = (oldQuery) => {
  ...
  const newConditions = oldConditions.map((condition) => {
    ...
    return condition.delete(logicalOp).set(key, new Map({ [comparisonOp]: values }));
  });
...
const convertVisualisations = async () => {
  ...
  for (const visualisation of visualisations) {
    try {
      ...
      logger.error(`Error updating Visualisation ${visualisation._id}`, err);
    }
  }
};
...
const convertDashboards = async () => {
  ...
  for (const dashboard of dashboards) {
    try {
      ...
      lodash.zip(oldShareableList, newShareableList).forEach(([oldShareable, newShareable]) => {
      ...
      logger.error(`Error updating Dashboard ${dashboard._id}`, err);
    }
  }
};
...
const convertStatementForwarding = async () => {
  ...
  for (const statementForwarding of statementForwardings) {
    try {
      ...
      logger.error(`Error updating SF ${statementForwarding._id}`, err);
    }
  }
};
...
const convertUsers = async () => {
  ...
  for (const user of users) {
    try {
      ...
      logger.error(`Error updating User ${user._id}`, err);
    }
  }
};
...
const convertQueries = async () => {
  ...
  for (const query of queries) {
    try {
      ...
      logger.error(`Error updating Query ${query._id}`, err);
    }
  }
};
