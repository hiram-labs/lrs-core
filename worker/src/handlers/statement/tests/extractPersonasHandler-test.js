...
import extractPersonasHandler, { extractPersonasStatementHandler } from '../extractPersonasHandler';
...
describe('Extract persona handler', () => {
  ...
  const testStatement = {
    ...
    processingQueues: [STATEMENT_EXTRACT_PERSONAS_QUEUE],
    completedQueues: [STATEMENT_QUERYBUILDERCACHE_QUEUE, STATEMENT_JOURNEY_QUEUE, STATEMENT_FORWARDING_QUEUE]
  };
  ...
  it('Should extract a persona if no presona or identifier exists', async () => {
    ...
    await Promise.promisify(extractPersonasHandler(personaService))({ statementId });
  ...
  it('Should add a persona to an identifier if the identifier and persona exists', async () => {
    await Statement.create(testStatement);
  ...
  it('should update matching statements with new persona id and identifier id', async () => {
    ...
    // RUN
    await Promise.promisify(extractPersonasHandler(personaService))({ statementId: queueStatementId });
    ...
    expect(statement1.person._id.toString()).to.equal(statement2.person._id.toString());
    expect(statement1.personaIdentifier.toString()).to.equal(statement2.personaIdentifier.toString());
    expect(statement1.person.display).to.equal(statement2.person.display);
  }).timeout(5000);
});
