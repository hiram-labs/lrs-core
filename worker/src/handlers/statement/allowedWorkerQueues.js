...
const getAllowedWorkerQueues = (allowedWorkerQueuesString) => {
  ...
  return allowedWorkerQueuesString.split(',').reduce((acc, queueString) => {
    if (allowableWorkerQueues.includes(queueString)) {
      return acc.concat([queueString]);
    }
    logger.warn(
      `"${queueString}" is ignored as an allowed worker queue. Allowable worker queues are ${allowableWorkerQueues.map((q) => `"${q}"`).join(', ')}`
    );
    return acc;
  }, []);
};
