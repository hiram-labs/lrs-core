...
import { keys, reject, includes, intersection, size } from 'lodash';
...
export const addStatementToPendingQueues = (statement, passedQueues, done) => {
  ...
  return Statement.updateOne({ _id: statement._id }, { $addToSet: { processingQueues: { $each: pendingQueueNames } } })
    .then(() =>
      // adding to queue returns a promise
      // turns queue names into a stream of promises
      // call done when they have all completed
      highland(pendingQueueNames)
        .flatMap((queueName) => {
...
export default ({ status, statementId }, jobDone) => {
  try {
    if (status) {
      ...
      return Statement.updateOne(idFilter, {
    ...
    return Statement.findById(statementId, { _id: 1, completedQueues: 1, processingQueues: 1 }).then((statement) => {
