...
export default () => {
  const subClient = redis.createClient('subscriber');
  const pubClient = redis.createClient('client');
        ...
        (cb) => {
          pubClient.rpop(pubKey, (err, payload) => {
            ...
            if (payload) {
              ...
              Statement.findOne({ 'statement.id': JSON.parse(payload).statementId }).then((statement) => {
