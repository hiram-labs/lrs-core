...
const handleStatement = (personaService) => async (statement) => {
  ...
  const personaName = statement.statement.actor.name ? statement.statement.actor.name : getIfiDisplayName(ifi);

  const { personaId, identifierId, wasCreated } = await personaService.createUpdateIdentifierPersona({
...
export const extractPersonasStatementHandler = (personaService) => (statements, done) =>
...
export default (personaService) =>
  wrapHandlerForStatement(STATEMENT_EXTRACT_PERSONAS_QUEUE, extractPersonasStatementHandler(personaService));
