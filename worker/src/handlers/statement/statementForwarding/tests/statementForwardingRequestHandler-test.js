...
describe('Statement Forwarding Request', () => {
  ...
  it('Request handles redirect', async () => {
    ...
    mock
      .onPost('http://redirectto/', { test: 'test' })
      .reply((config) => axios.post('http://localhost:3101/', { test: 'test' }, config));
  ...
  it('If a request fails, keep in pending', async () => {
    ...
    const doneStatement = await Statement.findById(statementId);
    expect(doneStatement.pendingForwardingQueue[0].toString()).to.equal(statementForwardingId);
    expect(doneStatement.completedForwardingQueue.length).to.equal(0);
    expect(doneStatement.failedForwardingLog[0].statementForwarding_id.toString()).to.equal(statementForwardingId);
