...
describe('Statement Forwarding handler', () => {
  it('Should take a statement and put it in a queue for each statementForwarder', () => {
    ...
    return new Promise((resolve, reject) => {
      async.parallel(
        {
          statementForwarding: (insertDone) =>
  ...
  it('Statement end to end', async () => {
      ...
      async.parallel(
        {
          ...
          statement: (insertDone) =>
            Statement.create({
              ...
              processingQueues: [],
              completedQueues: [
                // We're only interested in STATEMENT_FORWARDING_REQUEST_QUEUE
    ...
    */
    ...
    await Promise.all([StatementForwarding.deleteMany({}), Statement.deleteMany({})]);
  }).timeout(10000);
});
