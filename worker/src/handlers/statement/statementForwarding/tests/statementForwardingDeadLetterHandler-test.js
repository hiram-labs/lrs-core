...
describe('statementForwardingDeadLetterHandler', () => {
  it('Should update db', async () => {
    ...
    await new Promise((resolve) =>
      statementForwardingDeadLetterHandler(
        {
          statement,
          statementForwarding
        },
    ...
    await Statement.findById(statementId).then((statementModel) => {
      expect(statementModel.deadForwardingQueue[0].toString()).to.equal(statementForwardingId);
      expect(statementModel.pendingForwardingQueue.length).to.equal(0);
    });
