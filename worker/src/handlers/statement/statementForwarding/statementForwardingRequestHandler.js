...
import { STATEMENT_FORWARDING_REQUEST_DELAYED_QUEUE } from 'lib/constants/statements';
...
const sendRequest = async (statementToSend, statementForwarding, fullStatement) => {
  ...
  try {
    ...
    throw new ForwardingRequestError(message, {
      headers,
      responseBody,
      responseStatus,
      url
...
const statementForwardingRequestHandler = async ({ statement, statementForwarding }, done, { queue = Queue } = {}) => {
  try {
    await setPendingStatements(statement, statementForwarding._id);
    ...
    logger.debug(`SUCCESS sending statement ${statement._id} to ${statementForwarding.configuration.url}`);

    done();
  } catch (err) {
    logger.info(`FAILED sending statement ${statement._id} to ${statementForwarding.configuration.url}`, err);
    ...
    try {
      ...
      if (updatedStatement.failedForwardingLog.length <= statementForwarding.configuration.maxRetries) {
        ...
        queue.publish(
          ...
          (err) => {
            if (err) {
              logger.error(
                `FAILED sending statement ${updatedStatement._id} to ${STATEMENT_FORWARDING_REQUEST_DELAYED_QUEUE}`,
                err
              );
              done(err);
        ...
        logger.info(
          `EXCEEDED max retry for statement ${updatedStatement._id}, failing (should go to dead letter queue).`
        );
