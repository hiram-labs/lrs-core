...
const runSample = async (orgStatsList) => {
  // Sampling
  await Statement.aggregate([{ $sample: { size: SAMPLE_SIZE } }, { $out: 'statementSamples' }])
    .allowDiskUse(true)
    .exec();
  ...
  for (const orgStats of orgStatsList) {
    if (!orgStats.finished) {
      await StatementSample.aggregate([
      ...
      const orgSampleCount = await Statement.find(
        { organisation: orgStats.organisation },
        { _id: 0, organisation: 1 }
      ).count();
...
const saveOrganisationsWithUsageStats = async (organisations, orgStatsList) => {
  const orgStatsDict = orgStatsList.reduce((acc, s) => ({ ...acc, [s.organisation]: s }), {});
