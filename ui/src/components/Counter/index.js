...
const renderСontextLabel = ({ contextLabel, fontSize, color }) => (
  <ContextLabel style={{ fontSize, color }} key="contextLabel">
    {contextLabel}
  </ContextLabel>
);
  ...
  return (
    ...
    >
      <img
        ...
        src={percentage.icon}
      />
      {`${percentage.result} (${makeHumanReadable(model.get('previewPeriod', ''))})`}
    </div>
  );
};
...
const renderCounter = ({ color, results, model, height, width }) => {
  ...
  return (
    <div style={{ height: '100%' }}>
      <div style={{ width, fontSize: countFontsize, height: '100%', textAlign: 'center' }}>
        {renderedCount}
