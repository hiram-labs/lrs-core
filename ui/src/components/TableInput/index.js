...
const defaultInputComponent = ({ value, onChange }) => (
  <DebounceInput className="form-control" debounceTimeout={377} value={value} onChange={onChange} />
);
...
const withHandlersEnhance = withHandlers({
  onKeyChange:
    ...
    (element) => {
      ...
      const newValues = values.mapKeys((k) => (k === key ? newKey : k));
  ...
  onValueChange: (props) => (key) => (element) => {
    ...
    onChange(values.set(key, newValue));
  },
  remove:
    ({ values, onChange }) =>
    (key) =>
    () => {
      onChange(values.delete(key));
    },
  add:
    ({ values, onChange }) =>
    () => {
      onChange(values.set('New', ''));
    }
});
 ...
 */
const renderStaticValueHeaders = ({ staticValues }) =>
  staticValues
    .mapEntries(([value, key], index) => [
      index,
      <tr key={`static-value-header-${index}`}>
        <td>
          <input className="form-control" type="text" value={key} disabled />
        </td>
        <td>
 ...
 * @returns {immutable.Seq}
 */
const renderCustomHeaders = ({ values, onKeyChange, onValueChange, inputComponent = defaultInputComponent, remove }) =>
  values
    .filter((_, k) => k !== '_id')
    .mapEntries(([key, value], index) => [
      index,
      <tr key={`custom-header-${index}`}>
        <td>
          <DebounceInput className="form-control" debounceTimeout={377} value={key} onChange={onKeyChange(key)} />
        </td>
        <td>
          {inputComponent({
            key,
