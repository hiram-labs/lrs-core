...
const renderHead = ({ tHead, trsInTHead, fixTHead, tHeadTrsRefs, tHeadTrsTops, isWhiteStartingStripe }) => {
  if (!tHead) {
    return null;
  }
  ...
  return (
    <TableHead isWhiteStartingStripe={isWhiteStartingStripe}>
      {trsInTHead.map((tr, i) => {
        const childStyle =
          fixTHead && _.isNumber(tHeadTrsTops[i])
            ? {
                position: 'sticky',
                top: tHeadTrsTops[i]
              }
        ...
        return (
          <tr key={tr.key || i} ref={tHeadTrsRefs[i]}>
            {tr.props.children
              .flat()
              .filter((cell) => ['td', 'th'].some((t) => t === cell.type))
              .map((cell, j) =>
                React.createElement(cell.type, {
                  ...cell.props,
...
const renderBody = ({ tBody, trsInTBody, isWhiteStartingStripe }) => {
  ...
  return <TableBody isWhiteStartingStripe={isWhiteStartingStripe}>{trsInTBody}</TableBody>;
};

const renderFooter = ({ tFoot, trsInTFoot, fixTFoot, tFootTrsBottoms, tFootTrsRefs, isWhiteStartingStripe }) => {
  if (!tFoot) {
    return null;
  }
  ...
  return (
    <TableFooter isWhiteStartingStripe={isWhiteStartingStripe}>
      {trsInTFoot.flat().map((tr, i) => {
        const childStyle =
          fixTFoot && _.isNumber(tFootTrsBottoms[i + 1])
            ? {
                position: 'sticky',
        ...
        return (
          <tr key={tr.key || i} ref={tFootTrsRefs[i]}>
            {tr.props.children
              .flat()
              .filter((cell) => ['td', 'th'].some((t) => t === cell.type))
              .map((cell, j) =>
 ...
 * @returns {boolean}
 */
const areSameHeights = (hs1, hs2) => hs1.length === hs2.length && hs1.every((h, i) => h === hs2[i]);
 ...
 */
const ScrollableTable = ({ fixTHead = false, fixTFoot = false, children = [] }) => {
  ...
  const tHeadTrsTops = tHeadTrsHeights.reduce(
    (acc, height) => {
  ...
  const tFootTrsBottoms = [...tFootTrsHeights].reverse().reduce(
    (acc, height) => {
