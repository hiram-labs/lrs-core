...
class BarChart extends Component {
  ...
  getDataChunk = (model) => (data) => (page) =>
    data.slice(model.get('barChartGroupingLimit') * page, model.get('barChartGroupingLimit') * (page + 1));
  ...
  getSortedData = (results) => (labels) => this.sortData(getResultsData(results)(labels));

  renderPrevButton = () => (
    <span style={{ alignSelf: 'flex-start', marginLeft: 10 }}>
      <Button raised label="Previous" onMouseUp={this.displayPrevPage} icon={<i className="icon ion-chevron-left" />} />
    </span>
  );

  renderNextButton = () => (
    <span style={{ marginRight: 10, marginLeft: 'auto' }}>
      <Button raised label="Next" onMouseUp={this.displayNextPage} icon={<i className="icon ion-chevron-right" />} />
    </span>
  );
      ...
      return (
        <div>
          ...
          <Chart
            ...
            <YAxis dataKey="cellId" tickFormatter={getShortModel(data)} type="category" width={90} />
            <XAxis type="number" />
  ...
  render = () => {
    ...
    return hasData(this.props.results) ? this.renderResults(model)(results)(colors)(labels)(stacked) : <NoData />;
  };
}
