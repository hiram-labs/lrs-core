...
const renderChart = (model, component, axesLabels, chartWrapperFn) => (
  <StyledChart>
    ...
    <XAxisLabel>
      <StyledXAxis>{axesLabels.xLabel || model.getIn(['axesgroup', 'searchString'], 'X-Axis')}</StyledXAxis>
    </XAxisLabel>
  </StyledChart>
);
...
const renderResults =
  ...
  (chartWrapperFn) =>
    renderChart(
      model,
      renderChartResults(colors)(labels, toggleHiddenSeries, hiddenSeries)(stacked)(results),
      axesLabels,
      chartWrapperFn
    );
...
export default compose(hiddenSeriesState)(
  ({
    ...
    hasData(results) ? (
      renderResults(results)(model)(colors)(labels, toggleHiddenSeries, hiddenSeries)(stacked)(axesLabels)(
        chartWrapperFn
      )
    ) : (
      <NoData />
