...
import { getResultsData, getChartData, hasData, getLongModel } from './Chart';
import { Chart as StyledChart } from './styled';
import PieTooltip from './PieTooltip';

const cellPadding = 5;

const getInnerRadius = (size) => (i) => (hole) => size * i + cellPadding * i + hole;

const getOuterRadius = (size) => (i) => (hole) => size * (i + 1) + cellPadding * i + hole;
...
const sortData = (data) => data.sortBy((e) => e.get('total'));
...
const renderTooltips = (labels) => (data) => (count) => (grouping) => (
  <Tooltip content={<PieTooltip display={getLongModel(data)} labels={labels} count={count} grouping={grouping} />} />
);
...
const renderPies = (labels) => (colors) => (data) => (isDonut) => (maxSize) =>
  labels.map(renderPie(colors)(data)(maxSize)(isDonut)).valueSeq();
      ...
      {colors.size > 1 &&
        colors.map((colour, i) => (
          <div key={i} style={{ fontSize: '0.9em' }}>
            ...
            />
            <span title={labels.get(i) || `Series ${i + 1}`}>
              {(labels.get(i) && labels.get(i).substr(0, 45)) || `Series ${i + 1}`}
            </span>
          </div>
        ))}
    </div>
