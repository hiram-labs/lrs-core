...
export const getLabel = (entryId) => {
  ...
  return entryId.map((item) => {
    ...
    const outLangs = map(navLangs, (lang) => allLanguages.get(lang, null)).filter((item2) => !isNull(item2));
...
const getGroupModel = (data) => (group) => data.getIn([group, 'model'], group);

const getMinAndMax = (counts) => [0, counts.max()];

const getTotal = (entry) => entry.get('total', 0);

const renderBar = (index) => (stacked) => (label) => (color) => (
  <Bar key={index} dataKey={`Series ${index + 1}`} fill={color} name={label} stackId={stacked ? 1 : index} />
);
...
export const getLongModel = memoize(
  (data) => memoize((group) => getGroupModel(data)(group)),
...
export const renderBars =
  ...
  (labels) =>
  (stacked) =>
    labels.map((label, i) => renderBar(i)(stacked)(label)(colors.get(i))).valueSeq();
