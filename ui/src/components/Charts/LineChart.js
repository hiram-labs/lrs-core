...
const renderLine = (colors) => (label, i) => (
  <Line key={i + 1} dataKey={`Series ${i + 1}`} type="monotone" dot={false} stroke={colors.get(i)} name={label} />
);

const renderLines = (labels) => (colors) => labels.map(renderLine(colors)).valueSeq();

const renderLineChart =
  (labels, toggleHiddenSeries, hiddenSeries) =>
  (colors) =>
  (data) =>
  ({ width, height }) => (
    <Chart
...
const renderChart = (component, axesLabels, chartWrapperFn, model) => (
  <StyledChart>
    <BarContainer>
      <StyledYAxis>{wrapLabel(axesLabels.yLabel || model.getIn(['axesvalue', 'searchString'], 'Y-Axis'))}</StyledYAxis>
      <ChartWrapper>{chartWrapperFn(component)}</ChartWrapper>
    </BarContainer>
    <XAxisLabel>
      <StyledXAxis>{axesLabels.xLabel || 'yyyy/mm/dd'}</StyledXAxis>
    </XAxisLabel>
  </StyledChart>
);
...
const renderResults =
  (results) => (labels, toggleHiddenSeries, hiddenSeries) => (colors) => (axesLabels) => (chartWrapperFn) => (model) =>
    renderChart(
      renderChartResults(labels, toggleHiddenSeries, hiddenSeries)(colors)(results),
      axesLabels,
      chartWrapperFn,
      model
    );
