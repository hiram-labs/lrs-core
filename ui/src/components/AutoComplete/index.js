...
class AutoComplete extends Component {
  ...
  constructor(props) {
    ...
    this.propagateInputChange = debounce(this.propagateInputChange, 400, { leading: true, trailing: true });
  }
    ...
    switch (e.keyCode) {
      ...
      case keyCodes.BACKSPACE:
        if (
          ...
          e.preventDefault();
          this.deleteValue(this.props.values.lastKeyOf(this.props.values.last()));
        }
        break;
      default:
        break;
  ...
  addValue = (option) => {
    ...
    const newValues = multi ? this.props.values.set(index, value) : new Map({ [index]: value });
  ...
  renderOption = (value) => {
    ...
    return <OptionListItem label={label} tooltip={tooltip} />;
  };
  ...
  renderSingleValue = (value) => {
    ...
    if (renderSingleValue) {
      return <div style={{ position: 'absolute', width: '100%' }}>{renderSingleValue(value)}</div>;
    }
  };
  ...
  renderValues = () => {
    ...
    return hasSingleValue && inputValue === '' && this.renderSingleValue(values.first());
  };
  ...
  render() {
    ...
    return (
      ...
      >
        <InputWrapper isOpen={focused} isBorderHidden={!focused && noBorder} onClick={this.focus}>
          {this.renderValues()}
          {this.renderInput()}
          {this.renderStatus()}
        </InputWrapper>
