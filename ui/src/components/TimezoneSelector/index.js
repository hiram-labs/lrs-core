...
export const buildDefaultOptionLabel = (orgTimezone) => `Organisation Default: ${timezoneLabels.get(orgTimezone)}`;
...
export const TimezoneSelector = ({ id, value, onChange, defaultOption, disabled }) => (
  ...
  >
    {defaultOption && <option value={DEFAULT}>{defaultOption.label}</option>}
    {options}
  </select>
);
