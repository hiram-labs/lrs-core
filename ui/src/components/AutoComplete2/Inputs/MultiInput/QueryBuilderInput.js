...
const convertPathToKeys = (path) => path.split('.');

const setValueAtPath = (model, value, path) => model.setIn(convertPathToKeys(path), value);

const convertValueToModel = (map) => map.reduce(setValueAtPath, new Map());

const removeObjectId = (value) => (Iterable.isIterable(value) ? value.get('$oid', value) : value);

const isEqAtPath = (model) => (value, path) => model.getIn(convertPathToKeys(path)) === removeObjectId(value);

const isValueNotEqModel = (value) => (model) => value.size !== value.takeWhile(isEqAtPath(model)).size;
 ...
 */
const convertValuesToModel = (values) => (models) => values.map(getModelByValue(models));
...
const defaultSearchStringToFilter = (searchString) => {
  switch (searchString) {
    case '':
      return new Map();
    default:
      return fromJS({ searchString: { $regex: searchString, $options: 'i' } });
  }
};
...
const withRenderOption = withProps(({ parseOption, parseOptionTooltip, deselectOption }) => ({
