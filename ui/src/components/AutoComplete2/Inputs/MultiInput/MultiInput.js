...
const MultiInput = ({
  ...
  return (
    <InputWrapper className={wrapperClasses}>
      {selectedOptions.map((option, key) => renderOption({ option, key })).valueSeq()}
      <DebounceInput debounceTimeout={377} value={searchString} onChange={onChangeSearchString} onFocus={onFocus} />
    </InputWrapper>
  );
};
