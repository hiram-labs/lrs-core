...
const checkInputFocus = componentDidUpdate(({ hasFocus, selectedOption, inputFocused, setInputFocusedFalse }) => {
...
const SingleInput = ({
  ...
  const value = inputFocused ? searchString : parseOption(selectedOption);
  ...
  return (
    <InputWrapper isFocused={hasFocus}>
      ...
      {!shouldRenderSearch && renderOption({ option: selectedOption })}
    </InputWrapper>
  );
};

const enhanced = compose(withFocusState, withFocusStateHandlers, checkInputFocus);
