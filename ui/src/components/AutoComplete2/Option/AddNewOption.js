...
const withEventHandlers = withHandlers({
  onSubmit:
    ({ addModel, onAddNew = () => {}, setIsEditingFalse, userId, defaultNewValues }) =>
    async (payload) => {
      setIsEditingFalse();
...
export default compose(
  ...
  withIsEditing,
  withEventHandlers
)(({ isEditing, setIsEditingTrue, onSubmit, onCancel, fields = ['name'], defaultValues = [] }) => {
  if (isEditing) {
    return <TextInputGroup onSubmit={onSubmit} onCancel={onCancel} fields={fields} defaultValues={defaultValues} />;
  }
