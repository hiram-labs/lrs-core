...
const AutoComplete = ({
  ...
  return (
    <AutoCompleteWrapper onKeyPress={onKeyPress}>
      <FocusGroup onFocus={() => {}} onBlur={onBlurInput} hasFocus={hasInputFocus}>
        {renderInput({
          onFocus: () => setInputFocus(true),
          hasFocus: hasInputFocus
        })}
        <DropdownContainer>{hasInputFocus && renderOptions({ onKeyPress, onBlur: onBlurInput })}</DropdownContainer>
      </FocusGroup>
    </AutoCompleteWrapper>
  );
};
