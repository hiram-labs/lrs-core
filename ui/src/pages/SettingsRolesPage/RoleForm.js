...
import { chain, map, groupBy, startCase, lowerCase, difference } from 'lodash';
...
export const generateNewScopes = (oldScopes, scope, checked) => {
  ...
  if (checked) {
    ...
    if (difference(dependencies, oldScopes.toJS()).length === 0) {
      const dependenciesDependents = chain(dependencies).map(getScopeDependents()).flatten();

      newScopesAll = oldScopes.subtract(new Set([...dependencies, ...dependenciesDependents]));
    } else {
      newScopesAll = oldScopes;
    }

    const scopeDependents = getScopeDependents()(scope);
    newScopes = newScopesAll.subtract(new Set([scope, ...scopeDependents]));
  }
...
export const groupScopes = (scopes = orgScopes) => {
  const groupedOrgScopes = groupBy(scopes, (item) => [...item.split('/').slice(0, -1)].join('/'));

  return groupedOrgScopes;
};

const isChecked = (key, model) => model.get('scopes', new List()).includes(key);
...
const renderScopeTableRow = ({ key, scopes, handleScopeChange, model }) => (
  <tr key={`${key}_td`}>
