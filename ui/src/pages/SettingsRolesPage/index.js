...
const RoleDeleteButton = compose(withModel)(DeleteButton);
...
const enhance = compose(
  connect((state) => ({ searchString: modelQueryStringSelector(schema)(state) }), { addModel, clearModelsCache }),
  setPropTypes({
    searchString: PropTypes.string
  }),
  withHandlers({
    handleAddClick:
      ({ userId, addModel: createModel, clearModelsCache: clearCache }) =>
      async () => {
        try {
          await createModel({
            schema,
...
export const buttonFilter = (buttons, models) => (models.size === 1 ? [] : buttons);
    ...
    <div className="row">
      <div className="col-md-12">
        <RoleList
          ...
          getDescription={(model) => model.get('title') || '~ Unnamed Role'}
        />
      </div>
    </div>
  </div>
