...
describe('RoleForm', () => {
  ...
  it('Should remove dependent permissions', () => {
    const result = generateNewScopes(
      new Set([EDIT_PUBLIC_VISUALISATIONS, VIEW_PUBLIC_VISUALISATIONS]),
      EDIT_PUBLIC_VISUALISATIONS,
      false
    );
  ...
  it('Should not remove dependencies if only some dependencies are checked', () => {
    const result = generateNewScopes(new Set(scopes), EDIT_PUBLIC_VISUALISATIONS, false).toSet();

    const intersectSize = result.intersect(
      new Set([
        ALL,
        EDIT_ALL_VISUALISATIONS,
        VIEW_ALL_VISUALISATIONS,
    ...
    const result2 = generateNewScopes(result.toSet(), EDIT_PUBLIC_VISUALISATIONS, false).toSet();
  ...
  it('selecting a checkbox should select all its dependencies', () => {
    ...
    expect(result.toSet().subtract(new Set([EDIT_PUBLIC_VISUALISATIONS, VIEW_PUBLIC_VISUALISATIONS])).size).toEqual(0);
  });
});
