...
const SiteUserOrgItems = ({ models, user, filter }) => {
  ...
  const orgItems = filter
    ...
    .get('$in')
    .map((org) => {
      const orgModel = models.find((model) => model.get('_id').toString() === org.get('$oid').toString());

      if (orgModel === undefined) {
        return <li key={org.get('$oid').toString()}>Sorry organisation with id {org.get('$oid')} was deleted!</li>;
      }
