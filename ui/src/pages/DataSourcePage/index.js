...
import { statementQuerySelector, statementTimezoneSelector } from 'ui/redux/selectors';
...
const withStatements = compose(withProps({ schema: 'statement', displayOwner: false }), withModels);
...
class Source extends PureComponent {
  ...
  displayStatement = (statement = new Map()) => <Statement statement={statement} setFilterAt={this.setFilterAt} />;

  render = () => {
    const pipelines = fromJS([[{ $match: this.props.query }]]);
      ...
      <div>
        <header id="topbar">
          <Heading>
            Statements
            <HeadingButtons>
              <TooltipIconButton tooltip="Open export panel" tooltipPosition="left" onClick={this.toggleIsExporting}>
                <HeadingIcon className="ion-android-download" />
              </TooltipIconButton>
            </HeadingButtons>
          </Heading>
        </header>
        <VelocityTransitionGroup component="div" leave={{ animation: 'slideUp', duration: 350 }}>
