...
class ExportForm extends Component {
  ...
  postDownloadExport = () => {
    ...
    this.props.downloadExport({ exportId: this.props.id, pipelines: [this.getCompletePipeline()] }).then(() => {
  ...
  getActiveProjection = (state, props) =>
    props.model.getIn(['projections', state.activeIndex], getDefaultProjectionFromType(props.model.get('type')));
  ...
  renderDownloadButton = () => {
    switch (this.props.downloadRequestState) {
      ...
      case downloadRequestStates.default:
        return (
          <button className="btn btn-default pull-right" onClick={this.onDownloadExport}>
            Download
          </button>
        );
      case downloadRequestStates.waiting:
        return (
          <button className="btn btn-default pull-right" disabled>
            <i className="ion ion-clock" /> Requesting download...
          </button>
        );
      case downloadRequestStates.inqueue:
        return (
          <div className="alert alert-success pull-right">Your download will be available in the Downloads tab.</div>
        );
    }
  };
  ...
  render = () => {
    ...
    return (
      <div>
        <div className="row rowadding" style={{ margin: '10px 0', display: 'flex' }}>
          {rawMode ? (
            <div className="col-xs-12">
              <ProjectionInput projection={activeProjection} onChange={this.onUpdateProjection} rawMode />
            </div>
          ) : (
            [
              <ProjectionInputSection className="col-md-6 col-xs-12" key="export-projection">
                <ProjectionInput projection={activeProjection} onChange={this.onChangeProjection} />
        ...
        <button className="btn btn-default" onClick={this.toggleRaw}>
