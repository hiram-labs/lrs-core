...
const enhance = compose(
    ...
    const results = models
      .map((model) =>
        project.map((value, key) =>
          isString(value) ? model.getIn(value.replace('$', '').split('.')) : model.getIn(key.split('.'))
        )
      )
      .toList();
  ...
  shouldUpdate(
    (props, nextProps) => !(props.results.equals(nextProps.results) && props.headings.equals(nextProps.headings))
  ),
  ...
  ({ columnIndex, rowIndex, style, key }) => {
    ...
    return (
      <Cell key={key} style={style}>
        <CellWrapper title={display}>{display}</CellWrapper>
      </Cell>
    );
  };
