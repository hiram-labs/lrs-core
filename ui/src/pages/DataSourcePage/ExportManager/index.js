...
const enhance = compose(
  ...
  withHandlers({
    onAddExport:
      ({ addModel, userId, setSelectedExportId }) =>
      async () => {
...
const renderSelectedExport = ({ pipelines, selectedExportId, setSelectedExport }) => (
  <div>
    ...
    <ExportForm key={1} id={selectedExportId} pipelines={pipelines} />
  </div>
);

const renderNoExport = (onAddExport) => (
  <AddModelPrompt message="No exports configured yet! Add one to get started." schema={schema} onAdd={onAddExport} />
);
...
const render = ({
  ...
  <Tabs index={activeTab} onChange={setActiveTab}>
    <Tab label="Manage">
      <div>{renderExports({ isLoading, models, pipelines, selectedExportId, setSelectedExport, onAddExport })}</div>
    </Tab>
    <Tab label="Downloads">
