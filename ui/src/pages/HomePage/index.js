...
class Home extends Component {
  ...
  renderOrg = ({ key, style, index }) => {
    ...
    if (models.has(index)) {
      ...
      if (isSiteAdmin) {
        rightActions.push(<OrgMemberButton key="member" schema="organisation" id={organisation.get('_id')} />);
      }
  ...
  renderNoUserOrgs = () => {
    const { isLoading } = this.props;
    if (isLoading) return <Spinner />;
    return <div style={{ marginTop: '5px', textAlign: 'center' }}>You have not been added to any organisations.</div>;
  };
