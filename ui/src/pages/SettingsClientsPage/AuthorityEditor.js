...
const enhance = compose(
  ...
  shouldUpdate((prev, next) => !prev.authority.equals(next.authority)),
  withHandlers({
    handleIfiChange:
...
const render = ({ authority, handleFieldChange, handleIfiChange }) => (
  <div>
    <div className="form-group">{renderIfiSelector({ authority, handleIfiChange })}</div>
    ...
    <div>{renderIfiEditor({ authority, handleFieldChange })}</div>
  </div>
);
