...
const renderStoreItems = (stores) =>
  stores.valueSeq().map((store) => ({ value: store.get('_id'), label: store.get('title') }));
...
const renderStoreDropdown = compose(
  ...
  <Dropdown
    ...
    onChange={onSelect}
    source={models.size > 0 ? [renderDefaultStore(), ...renderStoreItems(models)] : renderNoStore()}
  />
));
  ...
  return (
    <div className="row">
      <div className="col-md-12">
        ...
        <div className="form-group">
          <Checkbox label="Enabled?" onChange={handleAttrChange.bind(null, 'isTrusted')} checked={isTrusted} />
        </div>
        ...
        {lrsId && (
          <div>
            <div className="form-group">
              ...
              {renderScopes(scopes, onChangeScopes, getXapiScopes(enableSingleStatementDeletion))}
            </div>
            <div className="form-group">
