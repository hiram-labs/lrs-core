...
const getActiveOrgSettings = ({ model, organisationModel }) => {
  ...
  const settings = model.get(ORG_SETTINGS, new List()).find((val) => val.get('organisation').toString() === org);
...
const enhance = compose(
  ...
  withHandlers({
    handleRolesChange:
      ({ model, organisationModel, updateOrgSettings }) =>
      (role, checked) => {
        ...
        const newRoles = (checked ? rolesSet.add(role) : rolesSet.delete(role)).toList();
    ...
    handleSiteRolesChange:
      ...
      (role, checked) => {
        ...
        const newScopes = (checked ? scopesSet.add(role) : scopesSet.delete(role)).toList();
      ...
      <div className="row">
        <div className="col-md-12">
          <div className="form-group">
            ...
            <QueryBuilder
              id={filterId}
              componentPath={new List(['user', model.get('_id')])}
              query={filter}
