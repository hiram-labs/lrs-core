...
const deleteButtonWithDisabled =
  (models) =>
  ({ id, ...props }) => {
    const disabled = models.getIn([id, 'importStage']) === STAGE_PROCESSING;

    return <DeleteButton id={id} disabled={disabled} {...props} />;
  };
...
export const PersonasImportsComponent = ({ onAddModel: doOnAddModel }) => (
  <div>
    <header id="topbar">
      <div className="heading heading-light">
        <span className="pull-right open_panel_btn">
          <button className="btn btn-primary btn-sm" onClick={doOnAddModel}>
