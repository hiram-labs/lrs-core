...
import { uploadPersonas, requestStateSelector } from 'ui/redux/modules/persona';
...
export const UploadButtonComponent = ({ requestState, onSubmit, disabled = false }) => {
  switch (requestState) {
    case IN_PROGRESS:
      return (
        <button onClick={onSubmit} className="btn btn-primary pull-right">
          <i className="icon ion-gear-a animated rotate infinite" style={{ display: 'block' }} />
        </button>
      );
    case COMPLETED:
      return (
        <button onClick={onSubmit} disabled className="btn btn-primary pull-right">
    ...
    default: {
      return (
        <button onClick={onSubmit} className="btn btn-primary pull-right" disabled={disabled}>
...
const handlers = withHandlers({
  onSubmit:
    ({ model, file, uploadPersonas: doUploadPersonas }) =>
    (event) => {
...
export const InitialUploadComponent = ({ model, onSubmit, handleFileChange, file, disableUpload }) => (
  <div>
    ...
    <div className="form-group">
      <UploadButton onSubmit={onSubmit} id={model.get('_id')} disabled={disableUpload} />
    </div>
    <div className="form-group">{model.get('stage')}</div>
  </div>
);
