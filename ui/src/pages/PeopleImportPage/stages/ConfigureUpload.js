...
const handlers = withHandlers({
  handleImportPersonas:
    ({ model, importPersonas: doImportPersonas, setDisabled }) =>
    () => {
      setDisabled(true);
      doImportPersonas({
        id: model.get('_id')
...
export const ConfigureUploadComponent = ({
  ...
  return (
    <div>
      ...
      >
        <ColumnHeaderEditor model={model} disabled={disabled} />
      ...
      {!disabled && (
        <div className="form-group">
          <button className="btn btn-primary pull-right" onClick={handleImportPersonas}>
            Import Personas
          </button>
        </div>
      )}
