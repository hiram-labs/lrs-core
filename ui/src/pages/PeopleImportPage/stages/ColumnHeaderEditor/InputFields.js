...
const handlers = {
  onColumnTypeChange:
    ({ columnStructure, model, updateModel: doUpdateModel }) =>
    (event) => {
      ...
      const newPrimary = isOrderable ? personasImportHelpers.getPrimaryMaxPlusOne({ structure: newStructure }) : null;
  ...
  onUseConstantChange:
    ...
    (useConstant) => {
      ...
      let newStructure = model.get('structure').setIn([columnName, 'useConstant'], useConstant);
  ...
  onConstantChange:
    ...
    (constant) => {
      ...
      const newStructure = model.get('structure').setIn([columnName, 'constant'], constant);
  ...
  onAttributeNameChange:
    ...
    (attributeName) => {
      ...
      const newStructure = model.get('structure').setIn([columnName, 'attributeName'], attributeName);
...
const InputFields = ({
  ...
  return (
    <TableData style={{ display: 'flex', flexWrap: 'wrap', overflowX: 'auto' }}>
      ...
      {isColumnOrderable && (
        <PrimaryForm primary={columnStructure.get('primary')} onChange={onPrimaryOrderChange} disabled={disabled} />
      )}

      {columnStructure.get('columnType') === COLUMN_ACCOUNT_KEY && (
        <AccountNameForm relatedColumn={columnStructure.get('relatedColumn')} />
      )}
...
export default compose(
  connect((state) => state, { updateModel }),
