...
import { withModel, withPolling } from 'ui/utils/hocs';
import { List } from 'immutable';
import moment from 'moment';
import { formatUrl } from 'ui/utils/LLApiClient';
import { activeOrgIdSelector } from 'ui/redux/modules/router';
import { STAGE_UPLOAD, STAGE_CONFIGURE_FIELDS, STAGE_IMPORTED, STAGE_PROCESSING } from 'lib/constants/personasImport';
...
export const PersonasImportFormComponent = ({ model, changeTitle, organisationId }) => {
  ...
  return (
    <div>
      ...
      {model.get('importStage') === STAGE_UPLOAD && <InitialUploadForm className="initialUpload" model={model} />}
      {model.get('importStage') === STAGE_CONFIGURE_FIELDS && (
        <ConfigureUpload className="configureUpload" model={model} />
      )}
      {model.get('importStage') === STAGE_PROCESSING ||
          ...
          <div className="stageImported">
            ...
            <blockquote>
              ...
              <p>
                ...
                <b>
                  Created: {model.getIn(['result', 'created'])} new personas
                  <br />
                </b>
                {errorSize > 0 && (
                  <b style={{ color: 'red' }}>
