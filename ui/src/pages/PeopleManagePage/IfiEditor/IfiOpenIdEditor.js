...
const render = ({ value, onChange, onSave, refValueInput }) => {
  ...
  return (
    <div className={classNames({ 'has-error': hasOpenIdError })}>
      <Input value={value} placeholder="OpenID URL" onChange={onChange} onSubmit={onSave} inputRef={refValueInput} />
      <ErrorText hasError={value.length !== 0 && hasOpenIdError}>Must be a valid URL.</ErrorText>
    </div>
  );
};
