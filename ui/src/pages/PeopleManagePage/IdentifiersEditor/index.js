...
const TableHeader = styled.th`
  ${tableDataStyle}
`;
const TableActionsHeader = styled(TableHeader)`
  ${actionsStyle}
`;
...
const render = ({ personaId, models, isNewIdentifierVisible, changeNewIdentifierVisibility, addModel }) => {
  ...
  return (
    <div>
      <div style={{ textAlign: 'right', marginBottom: '8px' }}>
        <AddTextIconButton text="Add Identifier" onClick={() => changeNewIdentifierVisibility(true)} />
      </div>
      <table style={{ width: '100%' }}>
        ...
        <tbody>
          {!isNewIdentifierVisible ? null : <NewRow onAdd={handleNewRowAdd} onCancel={handleNewRowCancel} />}
          {models.map((model) => <SavedRow id={model.get('_id')} key={model.get('_id')} />).valueSeq()}
        </tbody>
      </table>
    </div>
  );
