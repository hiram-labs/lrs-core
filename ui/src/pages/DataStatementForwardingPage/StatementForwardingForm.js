...
const updateHandlers = withHandlers({
  ...
  changeHeaders:
    ...
    (newHeaders) => {
      ...
      return updateModel({
        path: ['configuration'],
        value: model.get('configuration', new Map()).set('headers', newHeadersString)
      });
    },
  changeFullDocument:
    ({ updateModel }) =>
        ...
        <div className="row">
          <div className="col-md-12">
            <div className="form-group">
              <Switch label="Active?" onChange={changeActive} checked={model.get('active', false)} />
            </div>
            ...
            <div className="form-group">
              ...
              <span className="help-block">
                Should the Statement Forward send the entire database record to the defined URL? If forwarding to an
                LRS, this option should be left disabled.
              </span>
              <Switch
            ...
            >
              <label
                htmlFor={`${model.get('_id')}maxRetries`}
              >{`Max Retries (0 - ${STATAMENT_FORWARDING_MAX_RETRIES})`}</label>
              <input
