...
const renderHtml = (req) => (store) => {
  ...
  data.scripts = [clientAssets.vendor.js, clientAssets.client.js];
...
export default (req, res) => {
  ...
  Dashboard.findById(dashboardId)
    .then((dashboard) => {
      ...
      if (shareableId) {
        shareableDashboard = find(dashboard.shareable, (share) => share._id.toString() === shareableId);
    ...
    .then((dashboard) =>
      createDashboardJWT(dashboard, shareableId, 'native').then((token) => {
          ...
          case sharingScopes.VALID_DOMAINS: {
            ...
            if (invalidReferer) {
              ...
              if (siteIsntReferer) {
                return res
                  .status(404)
                  .send('Dashboard can only be embedded within valid domains, or opened directly from Learning Locker');
              }
            }
