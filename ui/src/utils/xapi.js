 ...
 */
export const getActorIdentifiers = (actor = new Map()) => actor.filter((value, key) => actorIdents.has(key));

export const displayLangMap = (langMap = new Map(), defaultDisplay = '') => {
  const langs = (navigator && navigator.languages) || [];
  const displays = langs.map((lang) => langMap.get(lang)).filter((value) => !!value);
  const display = displays[0] || langMap.first();
  return display || defaultDisplay;
};

export const displayVerb = (verb = new Map()) => displayLangMap(verb.get('display', new Map()), verb.get('id'));
