...
const getNextLabel = (criteria) => {
  const labels = criteria
    .map((criterion, criterionKey) => criterionKey.get('criterionLabel'))
    .toList()
    .sort();
...
const getCriteriaFromList = (list, criteria) =>
  list.reduce((result, queryValue) => getCriteriaFromQuery(queryValue, result), criteria);
...
const getCriteriaFromMap = (map, criteria) => {
  ...
  return keys.reduce((result, key) => getCriteriaFromQuery(map.get(key), result), criteria);
};
 ...
 * @returns {immutable.Map}
 */
const getMemoizedCriteria = memoize(
  (args) => {
 ...
 * @returns {immutable.Map}
 */
export const getCriteria = (query) => getCriteriaFromQuery(query, new Map());

export const changeCriteria = (criteria) => new Map(criteria.count() > 0 ? { $and: criteria.toList() } : {});
...
export const queryToStringList = (query) => {
  const criteria = getCriteria(query);
  const output = criteria.reduce(
    (description, criterion, criterionKey) => description.push(criterionToString(criterion, criterionKey)),
    new List()
  );
  return output;
};
