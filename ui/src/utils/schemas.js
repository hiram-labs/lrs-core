...
class LLSchema extends Schema {
  constructor(key, props, { reviver = defaultReviver, validate, preSave, editableFields } = {}) {
    super(key, props);
...
const user = new LLSchema(
  'user',
  { idAttribute: '_id', sortKey: 'updatedAt' },
  {
    preSave: (model) =>
...
const visualisation = new LLSchema(
  'visualisation',
  { idAttribute: '_id', sortKey: 'updatedAt' },
  {
    preSave: (model) =>
      model
        .update('filters', fromJS([{ $match: {} }]), (filters) => filters.map((query) => JSON.stringify(query.toJS())))
...
const statementForwarding = new LLSchema(
  ...
  {
    preSave: (model) => model.update('query', fromJS({}), (query) => JSON.stringify(query.toJS())),
    reviver: (key, value) => {
      let out = value.toMap();
...
const query = new LLSchema(
  'query',
  { idAttribute: '_id', sortKey: 'updatedAt' },
  {
    preSave: (model) =>
      model.update('conditions', new Map({}), (conditions) =>
        conditions.size > 0 ? JSON.stringify(conditions.toJS()) : JSON.stringify(conditions)
      )
  }
);

const personaIdentifier = new LLSchema(
  'personaIdentifier',
...
const personaAttribute = new LLSchema(
  'personaAttribute',
...
const client = new LLSchema(
  'client',
  { idAttribute: '_id', sortKey: 'updatedAt' },
  {
    preSave: (model) =>
      model.update('authority', (authority) => (Map.isMap(authority) ? JSON.stringify(authority.toJS()) : undefined))
  }
);

const dashboard = new LLSchema(
  'dashboard',
  { idAttribute: '_id' },
  {
    preSave: (model) => {
...
const exportSchema = new LLSchema(
  'export',
  { idAttribute: '_id' },
  {
    // export is a reserved word
    preSave: (model) =>
      model.update('projections', new List(), (conditions) =>
        conditions.map((condition) => JSON.stringify(condition.toJS()))
      ),
    reviver: (key, value) => {
      const isIndexed = Iterable.isIndexed(value); // From default reviver.
      if (key === 'projections') {
...
const querybuildercachevalue = new LLSchema(
...
const aggregation = new LLSchema('aggregation', {
  idAttribute: (model) => (isString(model._id) ? model._id : JSON.stringify(model._id))
});
