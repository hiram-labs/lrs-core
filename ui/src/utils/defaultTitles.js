...
export const createDefaultTitle = (model) => {
  ...
  const addXY = (selectedX, selectedY) =>
    `X: ${startCase(toLower(selectedX))} Y: ${makeOperatorReadable(model, 'axesoperator')}${startCase(toLower(selectedY))}`;
  const addXVSYXY = (selectedX, selectedY) =>
    `X: ${makeOperatorReadable(model, 'axesxOperator')} ${startCase(toLower(selectedX))} Y: ${makeOperatorReadable(model, 'axesyOperator')}${startCase(toLower(selectedY))}`;
  const addYX = (selectedX, selectedY) =>
    `X: ${startCase(toLower(selectedY))} Y: ${makeOperatorReadable(model, 'axesoperator')}${startCase(toLower(selectedX))}`;
...
export const createDefaultTitleWithIcon = (model) => (
  <span>
    <VisualiseIcon type={model.get('type')} sourceView={model.get('sourceView', false)} />

    <span style={{ marginLeft: '3px' }}>
      {model.get('description') || (
        <span style={{ color: '#B9B9B9', fontWeight: '100' }}>{createDefaultTitle(model)}</span>
      )}
    </span>
  </span>
);
