...
class _LLApiClient {
  ...
  countModels = (schema, filter, params) =>
    get({
      url:
        schema === 'statement'
          ? formatUrl(routes.STATEMENTS_COUNT)
          : formatUrl(`${routes.RESTIFY_PREFIX}/${schema}/count`),
      headers: {
        Authorization: `Bearer ${this.getToken()}`
      },
      query: {
  ...
  postModel = (schema, props, params = {}) => post(this.constructRequest(schema, params, props, props));

  patchModel = (schema, props, params) => patch(this.constructRequest(schema, params, props, props));

  deleteModel = (schema, props, params) => del(this.constructRequest(schema, params, props, props));
  ...
  aggregateAsync = (pipeline, limit, sinceAt) =>
    get({
  ...
  downloadExport = ({ exportId, pipelines }) =>
    get({
  ...
  assignPersona = (personaId, personaIdentifierId) =>
    post({
  ...
  createPersonaFromIdentifier = (personaIdentifierId) =>
    post({
  ...
  removeOrganisationFromUser = ({ userId, organisationId }) =>
    del({
  ...
  updateUserOrganisationSettings = ({ userId, organisationId, values }) =>
    patch({
