 ...
 */
const getCookieName = ({ tokenType, tokenId }) => `${COOKIE_ACCESS_TOKEN}_${tokenType}${tokenId ? `_${tokenId}` : ''}`;

const getCookieNameStartsWith = ({ tokenType }, cookies) => {
  const key = findKey(cookies, (item, key2) => key2.startsWith(`${COOKIE_ACCESS_TOKEN}_${tokenType}_`));
...
const getCookieOptions = () => {
  ...
  if (process.env.COOKIE_DOMAIN) {
    cookieOptions.domain = process.env.COOKIE_DOMAIN.replace(/HOSTNAME/, location.hostname);
  }
...
export { getCookieName, getCookieNameStartsWith, getCookieOptions, testCookieName, testOrgCookieName };
