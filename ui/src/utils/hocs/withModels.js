...
import { modelsByFilterSelector, isLoadingSelector, hasMoreSelector, shouldFetchSelector } from 'ui/redux/selectors';
import {
  fetchModels,
...
export default (WrappedComponent) => {
  class WithModels extends Component {
    ...
    fetchModels = ({ schema, filter, sort, first }) => {
      if (filter) {
        this.props.fetchAllOutstandingModels({ schema, filter, sort, first }).catch((err) => {
          console.error(err);
        });
      }
    };
  ...
  return connect(
    (state, { schema, filter, sort, cursor }) => ({
    ...
    { fetchModels, fetchMore, addModel, deleteModel, updateModel, saveModel, fetchAllOutstandingModels }
  )(WithModels);
};
