...
const createPersonaAttributeStages = ({ groupType }) => {
  ...
  const lookupStage = createStagePipeline('$lookup', {
    ...
    pipeline: [
      { $match: { $expr: { $and: [{ $eq: ['$personaId', '$$personaId'] }, { $eq: ['$key', personaAttrsKey] }] } } }
    ]
  });
  ...
  return lookupStage.concat(existsMatchStage).concat(unwindStage).concat(matchAttrsKeyStage);
};
...
export default ({ valueType, groupType, operatorType, timezone, contextActivityDefinitionType }) => {
  ...
  const contextActivityTypeMatchStage = contextActivityTypeMatch
    ? createStagePipeline('$match', contextActivityTypeMatch)
    : [];
  ...
  // Concat all stages
  return preReqs.concat(contextActivityTypeMatchStage).concat(projectStage).concat(countPipeline).concat(postReqs);
};
