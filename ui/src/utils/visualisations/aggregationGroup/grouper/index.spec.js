...
describe('aggregationGroup grouper', () => {
  test('aggregationGroup grouper adds unwind pipeline stage', () => {
    ...
    expect(result.getIn([1, '$unwind']).toJS()).toEqual({ path: '$statement.context.contextActivities.category' });
  });
  ...
  describe('when groupType is "date"', () => {
    ...
    test('pipeline has correct stages', () => {
      expect(result.toJS().map((stage) => Object.keys(stage)[0])).toEqual([
        '$project',
        '$group',
        '$sort',
        '$limit',
  ...
  describe('when groupType starts with "persona.import."', () => {
    ...
    test('pipeline content is correct', () => {
      expect(result.getIn([0, '$lookup']).toJS()).toEqual({
        from: 'personaAttributes',
        as: 'personaAttrs',
        let: { personaId: '$person._id' },
        pipeline: [
          { $match: { $expr: { $and: [{ $eq: ['$personaId', '$$personaId'] }, { $eq: ['$key', 'abcde'] }] } } }
        ]
      });

      expect(result.getIn([1, '$match']).toJS()).toEqual({ personaAttrs: { $exists: true } });

      expect(result.getIn([2, '$unwind']).toJS()).toEqual({ path: '$personaAttrs' });

      expect(result.getIn([3, '$match']).toJS()).toEqual({ 'personaAttrs.key': 'abcde' });

      expect(result.getIn([4, '$project']).toJS()).toEqual({
