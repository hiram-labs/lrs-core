...
export default function createAsyncDuck({
  ...
  const wrapStartAction = (args) => (dispatch, getState) =>
  ...
  function* doActionSaga(args) {
    try {
      ...
      if (err.constructor === Unauthorised || (err.constructor === HttpError && err.status === 401)) {
        yield put(logoutActions.logoutAction());
      }
