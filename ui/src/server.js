...
app.use(helmet({ frameguard: config.enableFrameguard }));
...
if (!fs.existsSync(logDirectory)) mkdirp.sync(logDirectory);
...
app.use(
  Express.static(path.join(__dirname, '..', 'public'), {
    setHeaders: (res, path2) => {
      if (path2.match(/fonts\/.+\.woff2/)) res.setHeader('Content-Type', 'font/woff2');
    }
  })
);
  ...
  server.listen(config.port, (err) => {
    ...
    logger.info(
      '\n------------------------------------------------------------------\n' +
        ...
        '\n' +
        `==> 💻  Open ${process.env.SITE_URL} in a browser to view the app. UI port ${config.port}` +
        '\n------------------------------------------------------------------\n'
    );
