...
export const duck = createAsyncDuck({
  ...
  reduceStart: (state, { userId, organisationId }) =>
    state.setIn([`${userId}-${organisationId}`, 'requestState'], IN_PROGRESS),
  reduceSuccess: (state, { userId, organisationId }) =>
    state.setIn([`${userId}-${organisationId}`, 'requestState'], COMPLETED),
  reduceFailure: (state, { userId, organisationId }) =>
    state.setIn([`${userId}-${organisationId}`, 'requestState'], FAILED),
  ...
  doAction: function* doAction({ userId, organisationId, values, llClient }) {
    ...
    const { status, body } = yield call(llClient.updateUserOrganisationSettings, {
      userId,
      organisationId,
