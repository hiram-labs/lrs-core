...
const refreshRequestStateSelector = (tokenType) =>
  createSelector([authSelector], (auth) => auth.get(`${tokenType}RefreshRequestState`, false));

const shouldRefreshSelector = (tokenType) =>
  createSelector(
    [refreshRequestStateSelector(tokenType)],
    (refreshState) => refreshState !== IN_PROGRESS && refreshState !== COMPLETED && refreshState !== FAILED
  );

const userTokenSelector = createSelector([authSelector], (auth) => auth.getIn(['tokens', 'user'], null));

const activeOrgTokenSelector = createSelector([authSelector, activeOrgIdSelector], (auth, activeOrgId) =>
  auth.getIn(['tokens', 'organisation', activeOrgId], null)
);
...
const refreshToken = (type) =>
  createAsyncDuck({
    ...
    reduceFailure: (state, { error }) =>
      state.set(`${type}RefreshRequestState`, FAILED).set('refreshError', error.message),
