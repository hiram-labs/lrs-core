...
const requestResetErrorSelector = createSelector(authSelector, (auth) => auth.get('requestResetError'));

const requestResetRequestStateSelector = createSelector(authSelector, (auth) =>
  auth.get('passwordRequestResetState', false)
);

const shouldResetSelector = createSelector(
  [requestResetRequestStateSelector],
  (resetState) => resetState !== IN_PROGRESS && resetState !== COMPLETED && resetState !== FAILED
);
...
const requestResetPassword = createAsyncDuck({
  ...
  reduceFailure: (state, { error }) =>
    state.set('passwordRequestResetState', FAILED).set('requestResetError', error.message),
