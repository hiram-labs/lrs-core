...
export const isAuthenticatedSelector = (type = 'user', id) =>
  createSelector(tokenDuck.selectors.authTokenSelector(type, id), (token) => !!token);

export const loggedInUserId = createSelector(
  (state) =>
    tokenDuck.selectors.authSelector(state.auth.get('activeTokenType', 'user'), state.auth.get('activeTokenId'))(state),
  (authState) => authState.get('userId')
);

export const loggedInUserSelector = createSelector([(state) => state, loggedInUserId], (state, userId) =>
  modelsSchemaIdSelector('user', userId)(state)
);

export const currentScopesSelector = createSelector(tokenDuck.selectors.activeAuthSelector, (authState) =>
  authState.get('scopes', new List())
);

export const hasScopeSelector = (scope) =>
  createSelector(currentScopesSelector, (activeScopes) => activeScopes.includes(scope));

export const organisationSettingsSelector = createSelector(loggedInUserSelector, (user) =>
  user.get('organisationSettings')
);
...
export const activeOrganisationSettingsSelector = createSelector(
  ...
  (user, orgId, organisationSettings) => {
    ...
    return organisationSettings.find((os) => os.get('organisation', '').toString() === orgId.toString());
  }
);

export const isSiteAdminSelector = createSelector(currentScopesSelector, (currentScopes) =>
