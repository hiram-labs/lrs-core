...
const resetRequestStateSelector = createSelector([authSelector], (auth) => auth.get('passwordResetState', false));

const resetErrorSelector = createSelector([authSelector], (auth) => auth.get('resetError'));
...
const resetPassword = createAsyncDuck({
  ...
  reduceFailure: (state, { error }) => state.set('passwordResetState', FAILED).set('resetError', error.message),
