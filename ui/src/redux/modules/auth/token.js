...
const authSelector = (tokenType = 'user', tokenId) =>
...
const authTokenSelector = (tokenType, tokenId) =>
  createSelector([authSelector(tokenType, tokenId)], (authState) => authState.get('token'));

const activeTokenTypeIdSelector = createSelector(stateSelector, (authState) => ({
  tokenType: authState.get('activeTokenType'),
  tokenId: authState.get('activeTokenId')
}));
...
const orgTimezoneSelector = createSelector(activeAuthSelector, (activeAuth) =>
  activeAuth.getIn(['extensions', 'orgTimezone'], null)
);
...
const decodeLoginTokenReducer = (state, { tokenType, tokenId, decoded, token }) => {
  ...
  if (tokenType === 'user') {
    return state.setIn(['tokens', 'user'], fromJS(decoded)).setIn(['tokens', 'user', 'token'], token);
  }
  return state
...
const setActiveTokenAction = (tokenType = 'user', tokenId) => ({ type: SET_ACTIVE_TOKEN, tokenType, tokenId });

const setActiveTokenReducer = (state, { tokenType, tokenId }) =>
  state.set('activeTokenType', tokenType).set('activeTokenId', tokenId);
...
export const sagas = [watchStoreTokenSaga];
