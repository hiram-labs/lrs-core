...
const loginRequestStateSelector = createSelector([authSelector], (auth) => auth.get('loginRequestState', false));

const loginErrorSelector = createSelector([authSelector], (auth) => auth.get('loginError'));

const shouldLoginSelector = createSelector(
  [loginRequestStateSelector],
  (loginState) => loginState !== IN_PROGRESS && loginState !== COMPLETED && loginState !== FAILED
);
...
const basicLogin = createAsyncDuck({
  ...
  doAction: function* loginSaga({ username, password }) {
    const { status, body } = yield post(`/api${routes.AUTH_JWT_PASSWORD}`).use(
      basicAuth(username.toLowerCase(), password)
    );
