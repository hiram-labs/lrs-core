...
export const getActorQuery = (actor) => {
  ...
  return result.delete('statement.actor.name').delete('statement.actor.objectType');
};
  ...
  if (Iterable.isIterable(value)) {
    if (value.has('objectType')) {
      ...
      switch (value.get('objectType')) {
        case 'Agent':
          return displayActor(value);
        case 'Group':
          return displayActor(value);
        case 'Activity':
          return displayActivity(value);
        default:
      }
    }
        ...
        childGenerators: new List([
          new Map({
            ...
            pathMatcher: (path) =>
              path.size > 1 &&
              path.get(0) === 'persona' &&
              (path.get(1).indexOf('statement') !== 0 || path.get(1) === 'statement.actor.account.homePage'),
            ...
            getModel: () => (query) =>
              new Map({
                value: query.get('person._id', new Map()).get('$personaIdent', new Map()).get('value')
              })
          })
        ])
      },
...
const flattenDeep = (object, path = new List()) => {
  if (Iterable.isIterable(object)) {
    return object.map((child, key) => flattenDeep(child, path.push(key))).flatten();
  }
...
export const defaultParser = (value) => {
  if (Iterable.isIterable(value)) {
    if (value.has('objectType')) {
      ...
      switch (value.get('objectType')) {
        case 'Agent':
          return getActorIdentifiers(value);
        default:
      }
    }
...
const getChildOverridesFromValueType = (valueType, generator, childPath) => {
  ...
  if (childPath.size === 4 && childPath.take(3).equals(new List(['statement', 'context', 'contextActivities']))) {
    return {
...
export const valueToCriteria = (basePath, value) => {
  ...
  const result = flatIdents
    .mapKeys((flatKey) => (flatKey === '' ? basePath : `${basePath}.${flatKey}`))
    .map((v) => fromJS(v));
  return result;
};
...
export const buildChildrenFromCaches = (caches) => (generator) => {
  ...
  return groupedCaches
    .map((groupedCache, keyPath) => {
      ...
      const childWithInput = hasInput
        ? buildInputChild(generator)(keyPath, groupedCache.first().get('valueType'))
        : new Map();
      const childWithChildren = new Map({
        children: hasChildren ? buildChildrenFromCaches(caches)(childGenerator) : new Map()
      });
...
const buildChildrenFromGenerators = (caches) => (generators) => {
  ...
  return results.reduce((res, result) => res.merge(result), new Map());
};
...
const buildSectionFromGenerators = (section) => (caches) => (generators) => {
  ...
  return section.update('children', new Map(), (children) => children.merge(generatedChildren));
};
...
const buildSectionFromCaches = memoize(
  (args = new Map()) => {
    ...
    let nextSection = section.update('children', new Map(), (children) =>
      children.map((child) =>
        buildSectionFromCaches(
          new Map({
            section: child,
            caches
          })
...
export const getIdentifiersFromMatch = (match, keyPath) => {
  ...
  const matchingSection = andSection.find((section) => section.get('$comment') === keyPathString, null, new Map());
...
const updateQueryWithNewValues = (match, criteriaPath, values) => {
  ...
  if (criteria.size > 0) {
    const matchKeyPath = foundIndex >= 0 ? ['$and', foundIndex] : ['$and', rootAnd.size];
    return nextMatch.setIn(
      matchKeyPath,
      fromJS({
...
export const updateQueryAtKeyPath = (query = new Map(), keyPath, valuesUpdater) => {
  ...
  const newQuery = updateQueryWithNewValues(query, keyPath, newValues);
  return newQuery;
};

export const getAvailableSections = (sections, filter) =>
  createSelector([modelsByFilterSelector('querybuildercache', filter)], (caches) =>
    sections.map((section) => buildSectionFromCaches(new Map({ section, caches })))
  );

export const getAvailableSection = (section, filter) =>
  createSelector([modelsByFilterSelector('querybuildercache', filter)], (caches) =>
