...
const copyVisualisations = async ({ dispatch, visualisations, userId }) => {
  const promises = visualisations
    .map((visualisation) =>
      dispatch(
        addModel({
          schema: 'visualisation',
          props: visualisation.filter((_, k) => !VIZ_IGNORING_COLUMN_SET.has(k)).set('owner', userId),
          isExpanded: false
        })
      )
    )
...
function* copyDashboard({ dashboard, visualisations, dispatch, organisationId, userId }) {
  ...
  const newVisualisations = yield call(copyVisualisations, { dispatch, visualisations: uniqueVisualisations, userId });
  ...
  const newWidgets = dashboard.get('widgets', new List()).map((widget) => {
  ...
  yield put(
    routerActions.navigateTo('organisation.data.dashboards.id', {
      organisationId,
