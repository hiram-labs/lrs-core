...
const reducePurgeSaveQueue = (state, { time }) =>
  state.update('saveQueue', (schemas) => schemas.map((ids) => ids.filter((queuedTime) => queuedTime > time)));

/**
 * SELECTORS
 */
const modelsSelector = (state) => state.models;
const saveQueueSelector = createSelector([modelsSelector], (models) => models.get('saveQueue', new Map()));
...
function* persistSaveQueue() {
  ...
  const pendingSaveArray = pendingSaves
    .map((schemaId) =>
      put(
        saveModel({
