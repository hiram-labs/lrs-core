...
const addStateSelector = (schema) =>
  createSelector([modelsSelector], (models) => models.getIn([schema, 'requestState'], false));

const shouldAddSelector = (schema) =>
  createSelector(
    [addStateSelector(schema)],
    (addState) => addState !== IN_PROGRESS && addState !== COMPLETED && addState !== FAILED
  );
...
const addModel = createAsyncDuck({
  ...
  doAction: function* fetchModelSaga({ schema, props, isExpanded = true, llClient }) {
    ...
    if (status >= 400) {
      ...
      if (body.code === 11000 && schema === 'personaIdentifier') {
        throw new Error('Another persona already uses that identifier and it cannot be used twice.');
      }
