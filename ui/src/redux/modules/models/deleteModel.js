...
const deleteStateSelector = (schema) =>
  createSelector([modelsSelector], (models) => models.getIn([schema, 'deleteState'], false));

const shouldDeleteSelector = (schema) =>
  createSelector(
    [deleteStateSelector(schema)],
    (deleteState) => deleteState !== IN_PROGRESS && deleteState !== COMPLETED && deleteState !== FAILED
  );
...
const deleteModel = createAsyncDuck({
  ...
  reduceSuccess: (state, { schema, id }) =>
    state.setIn([schema, id, 'deleteState'], COMPLETED).removeIn([schema, id, 'remoteCache']),
  reduceFailure: (state, { schema, id }) => state.setIn([schema, id, 'deleteState'], FAILED),
  reduceComplete: (state, { schema, id }) => state.setIn([schema, id, 'deleteState'], null),
