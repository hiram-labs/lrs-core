 ...
 */
const localUnsavedSchemaIdSelector = (schema, id) =>
  createSelector(modelsSelector, (models) => models.getIn([schema, id, 'localCache', 'unsaved']));

/**
 * Returns the the cache of local pending state for a schema and an id
 */
const localPendingSchemaIdSelector = (schema, id) =>
  createSelector(modelsSelector, (models) => models.getIn([schema, id, 'localCache', 'pending']));
 ...
 */
const localModelSchemaIdSelector = (schema, id) =>
  createSelector(
 ...
 * @param {String} id e.g '58b6e794824d4624fc68f6b3'
 */
const remoteModelSchemaIdSelector = (schema, id) =>
  createSelector(modelsSelector, (models) => models.getIn([schema, id, 'remoteCache']));
 ...
 * @param {String} id e.g '58b6e794824d4624fc68f6b3'
 */
const modelsSchemaIdSelector = (schema, id, { deep } = {}) =>
  createSelector(
 ...
 */
const modelsPickSelector = (schema, ids = new Set()) =>
  createSelector([(state) => state, idSelector(ids)], (state, actualIds) => {
 ...
 * @return {Immutable.Map}                   Map of objects of type [schema] matching the filter
 */
const modelsByFilterSelector = (schema, filter, sort) =>
  createSelector(
    [(state) => state, fetchModelsDuck.selectors.idsByFilterSelector(schema, filter, sort)],
    (state, ids) => modelsPickSelector(schema, ids)(state)
  );
