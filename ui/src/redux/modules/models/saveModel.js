...
const saveStateSelector = ({ schema, id }) =>
  createSelector(modelsSchemaIdSelector(schema, id), (model = new Map()) => model.get('requestState', false));

const shouldSaveSelector = ({ schema, id }) =>
  createSelector([saveStateSelector({ schema, id })], (saveState) => {
    if (!id) return false;
 ...
 */
export const saveModelSelector = ({ schema, id }) =>
  createSelector(modelsSchemaIdSelector(schema, id), (model) => {
...
const saveModel = createAsyncDuck({
  ...
  doAction: function* saveModelSaga({ schema, props, id, beforeSave, llClient }) {
    ...
    if (status >= 300) {
      ...
      if (has(body, 'errors') && has(body, 'name') && body.name === 'ValidationError') {
        ...
        each(fieldErrors, (val, key) => {
          const messages = val.name === 'CastError' ? ['Could not accept this value'] : val.message.split(', ');
