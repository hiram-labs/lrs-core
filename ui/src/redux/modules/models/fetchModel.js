...
const modelRequestStateSelector = ({ schema, id }) =>
...
const isLoadingModelSelector = ({ schema, id }) =>
  createSelector([modelRequestStateSelector({ schema, id })], (requestState) => requestState === IN_PROGRESS);
...
const fetchModel = createAsyncDuck({
  ...
  doAction: function* fetchModelSaga({ schema, id, llClient }) {
    ...
    const { status, body } = yield call(llClient.getModel, schema, id);
    if (status >= 300) {
