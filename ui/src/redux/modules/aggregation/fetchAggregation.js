...
const aggregationRequestStateSelector = (pipeline) =>
  createSelector(aggregationSelector, (aggregations) => aggregations.getIn([pipeline, 'requestState']));

const aggregationShouldFetchSelector = (pipeline) =>
  createSelector(
    aggregationRequestStateSelector(pipeline),
    (requestState) => requestState !== IN_PROGRESS && requestState !== COMPLETED && requestState !== FAILED
  );

const aggregationShouldRecallSelector = (pipeline) =>
  createSelector(aggregationSelector, (aggregations) => {
...
const fetchAggregation = createAsyncDuck({
  ...
  reduceStart: (state, { pipeline }) => state.setIn([pipeline, 'requestState'], IN_PROGRESS),
  ...
  reduceFailure: (state, { pipeline, err }) =>
    state.setIn([pipeline, 'requestState'], FAILED).setIn([pipeline, 'error'], err),

  reduceComplete: (state, { pipeline }) => state.setIn([pipeline, 'requestState'], COMPLETED),
