...
export const requestStateSelector = ({ id }) =>
  createSelector(rootSelector, (state) => state.getIn([id, 'requestState']));
...
function* uploadPersonasSaga() {
  while (true) {
    ...
    if (status > 200) {
      ...
      yield put(mergeEntitiesActions.mergeEntitiesAction(entities));
...
const importPersonasDuck = createAsyncDuck({
  ...
  successDelay: 2000,
  doAction: function* doAction({ id, llClient }) {
...
const handler = handleActions({
  [UPLOAD_PERSONAS]: (state, { id }) => state.setIn([id, 'requestState'], IN_PROGRESS),
  [UPLOAD_PERSONAS_SUCCESS]: (state, { id }) => state.setIn([id, 'requestState'], COMPLETED),
  [UPLOAD_PERSONAS_FAILURE]: (state, { id }) => state.setIn([id, 'requestState'], FAILED),
  [UPLOAD_PERSONAS_RESET]: (state, { id }) => state.deleteIn([id, 'requestState']),
  [importPersonasDuck.constants.start]: (state, { id }) => state.setIn([id, 'requestState'], IN_PROGRESS),
  [importPersonasDuck.constants.success]: (state, { id }) => state.setIn([id, 'requestState'], COMPLETED),
  [importPersonasDuck.constants.failure]: (state, { id }) => state.setIn([id, 'requestState'], FAILED),
  [importPersonasDuck.constants.complete]: (state, { id }) => state.deleteIn([id, 'requestState'])
});
...
export const sagas = [uploadPersonasSaga, ...importPersonasDuck.sagas];
