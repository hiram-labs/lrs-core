...
export const modelQueryStringSelector = (schema) =>
  createSelector([searchSelector], (search) => search.getIn([schema, 'query'], ''));
...
export function queryStringToQuery(queryString, schema) {
  ...
  switch (schema) {
    case 'persona':
      return fromJS({ name: { $regex: escapedStr, $options: 'i' } });
    case 'user':
      return fromJS({
        $or: [{ name: { $regex: escapedStr, $options: 'i' } }, { email: { $regex: escapedStr, $options: 'i' } }]
      });
    case 'organisation':
      return fromJS({ name: { $regex: escapedStr, $options: 'i' } });
    case 'client':
      return fromJS({ title: { $regex: escapedStr, $options: 'i' } });
    case 'lrs':
      return fromJS({
        $or: [{ title: { $regex: escapedStr, $options: 'i' } }, { description: { $regex: escapedStr, $options: 'i' } }]
      });
    default:
      return fromJS({ description: { $regex: escapedStr, $options: 'i' } });
  }
}
 ...
 * @return {Immutable.Map}
 */
export const currentQuerySelector = (schema) =>
  createSelector([modelQueryStringSelector(schema)], (queryString) => queryStringToQuery(queryString, schema));
 ...
 * @return {Immutable.Map}
 */
export const modelsByCurrentQuerySelector = (schema) =>
  createSelector([(state) => state, currentQuerySelector(schema)], (state, query) =>
    modelsByFilterSelector(schema, query)(state)
  );
