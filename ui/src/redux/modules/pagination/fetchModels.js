...
const paginationStateSelector = (schema, filter = defaultFilter, sort = defaultSort) =>
  createSelector(paginationSelector, (pagination) => pagination.getIn([schema, toImmutable(filter), sort], new Map()));
...
const cursorSelector = (schema, filter, sort, direction) =>
  createSelector(paginationPageInfoSelector(schema, toImmutable(filter), sort), (pageInfo) => {
    switch (direction) {
      case FORWARD:
...
const shouldFetchSelector = ({ schema, filter, sort, cursor }) =>
  createSelector(
    [requestStateSelector({ schema, filter, sort, cursor }), cachedAtSelector({ schema, filter, sort, cursor })],
...
const isLoadingSelector = (paginationArgs) =>
  createSelector(requestStateSelector(paginationArgs), (requestState) => requestState === IN_PROGRESS);

const isCompletedSelector = (paginationArgs) =>
  createSelector(requestStateSelector(paginationArgs), (requestState) => requestState === COMPLETED);

export const idsByFilterSelector = (schema, filter, sort) =>
  createSelector(paginationStateSelector(schema, toImmutable(filter), sort), (pagination) =>
    pagination.get('edges', new OrderedSet()).map((item) => item.get('id'))
  );
...
export const reduceSuccess = (
  ...
  const newEdges2 = new OrderedSet(diffEdges(newEdges, oldEdges, cursorKey, pageInfo, direction));
...
const fetchModels = createAsyncDuck({
  ...
  startAction: ({ schema, filter = defaultFilter, sort = defaultSort, direction, first, last, cursor }) => ({
    schema,
    filter,
    sort,
    direction,
  ...
  successAction: ({ schema, filter, sort, direction, ids, pageInfo, cursor, edges }) => ({
    schema,
    filter,
    sort,
    direction,
  ...
  doAction: function* fetchModelSaga({ schema, filter, sort, direction, cursor, llClient, first, last }) {
...
const fetchAllOutstandingModels =
  ...
  (dispatch, getState) => {
    ...
    const fetchStateRecurse = (cursor2) => {
      const result = dispatch(fetchModelsStart({ schema, filter, sort, direction, first, last, cursor: cursor2 }));
      ...
      const result2 = result.then((newState) => {
        ...
        if (direction === FORWARD && !newState.pageInfo.get('hasNextPage')) {
          return;
        }

        if (direction === BACKWARD && !newState.pageInfo.get('hasPreviousPage')) {
          return;
        }
...
const fetchMore =
  ({ schema, filter, sort, direction = FORWARD, first = 10, last }) =>
  (dispatch, getState) =>
    dispatch(
