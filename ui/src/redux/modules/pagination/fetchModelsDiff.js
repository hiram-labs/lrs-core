...
const diffEdges = (inputEdges, oldEdges, cursor, pageInfo, direction = FORWARD) => {
  ...
  if (!pageInfo.get('hasNextPage') && !pageInfo.get('hasPreviousPage') && inputEdges.size === 0) {
    return new List();
  }
  ...
  diff.scanDiff((aStart, aEnd, bStart, bEnd) => {
    const toInsert = inList.slice(aStart, aEnd).filter((item) => item !== undefined);
    ...
    if (aStart === 0 && bStart === 0 && bEnd === outRaw.length) {
      ...
      inc += toInc;
    } else if (aStart === aEnd && aEnd === inRaw.length && pageInfo.get('hasNextPage', true) && direction === FORWARD) {
    ...
    } else if (aStart === 0 && aEnd === 0 && cursor !== undefined && direction === FORWARD) {
      /*
    ...
    } else if (aEnd === inRaw.length && pageInfo.get('hasNextPage', true) && direction === FORWARD) {
      /*
