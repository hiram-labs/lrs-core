...
const handler = handleActions({
  [MERGE_PERSONA_START]: (state, { id }) => state.setIn([id, 'requestState'], IN_PROGRESS),
  [MERGE_PERSONA_SUCCESS]: (state, { id }) => state.setIn([id, 'requestState'], COMPLETED),
  [MERGE_PERSONA_FAILURE]: (state, { id }) => state.setIn([id, 'requestState'], FAILED),
  [MERGE_PERSONA_DONE]: (state, { id }) => state.setIn([id, 'requestState'], null)
});
...
function* mergePersonaSaga() {
  while (true) {
    const { mergePersonaFromId, mergePersonaToId, llClient } = yield take(MERGE_PERSONA);

    yield put(mergePersonaStart(mergePersonaToId));
    try {
      const { status, body } = yield call(llClient.mergePersona, mergePersonaFromId, mergePersonaToId);
...
export const sagas = [mergePersonaSaga];
