...
const handler = handleActions({
  [SET_STATEMENT_QUERY]: (state, action) => state.set('query', action.query),
  [SET_STATEMENT_TIMEZONE]: (state, action) => state.set('timezone', action.timezone)
});
 ...
 * Selectors
 */
export const statementQuerySelector = createSelector([(state) => state], (state) =>
  state.statements.get('query', new Map())
);
export const statementTimezoneSelector = createSelector([(state) => state], (state) =>
  state.statements.get('timezone')
);
