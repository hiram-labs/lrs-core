...
import { shouldFetchSelector, fetchModels, fetchModelsCount, countSelector } from 'ui/redux/modules/pagination';
import { modelsSchemaIdSelector, modelsByFilterSelector, updateModel } from 'ui/redux/modules/models';
...
import { TYPES, NONE, JOURNEY, JOURNEY_PROGRESS } from 'ui/utils/constants';
...
const dashboardShareableIdSelector = (state) => {
  const out = state.router && state.router.route && state.router.route.params && state.router.route.params.shareableId;
  return out;
};

const dashboardIdSelector = (state) => {
  const out = state.router && state.router.route && state.router.route.params && state.router.route.params.dashboardId;
  return out;
};

const routeNameSelector = (state) => {
  const out = state.router && state.router.route && state.router.route.name;
  return out;
};
...
const shareableDashboardFilterSelector = () =>
  createSelector(
    [
      metadataSelector,
      modelsSelector,
    ...
    (metadata, models, routeShareableId, routeDashboardId, routeName, filter) => {
      ...
      if (viewingDashboardExternally) {
        ...
        const theShare = theDashboard
          .get('remoteCache', new Map())
        ...
        if (theShare.get('filterMode', OFF) !== OFF && filter) {
          ...
          return [theShare.get('filter', new Map()), parsedFilter];
        }
...
export const visualisationPipelinesSelector = (
  ...
  createSelector(
    ...
    (visualisation, filter, organisationModel, orgTimezoneFromToken) => {
      ...
      const timezone =
        visualisation.get('timezone') || orgTimezoneFromToken || organisationModel.get('timezone', 'UTC');
      const queries = visualisation.get('filters', new List()).map((vFilter) => {
        ...
        const out = new Map({
          $match: new Map({
            $and: new List([vFilter.get('$match', new Map()), ...filter])
          })
        });
...
const shouldFetchPipelines = (pipelines, state) =>
  pipelines.reduce((reduction, pipeline) => reduction || shouldFetchPipeline(pipeline, state), false);

const shouldFetchSeries = (series, state) =>
  series.reduce((reduction, pipelines) => reduction || shouldFetchPipelines(pipelines, state), false);

const shouldFetchJourney = (pipelines, state) =>
  pipelines.reduce(
    (reduction, pipeline) => reduction || shouldFetchSelector('journeyProgress', pipeline)(state),
    false
  );
 ...
 * @return {Boolean}
 */
export const visualisationShouldFetchSelector = (visualisationId) =>
  createSelector(
    [
      identity,
    ...
    (state, pipelines, visualisation) => {
      switch (visualisation.get('type')) {
        case JOURNEY_PROGRESS:
          return shouldFetchJourney(pipelines, state);
        default:
          return shouldFetchSeries(pipelines, state);
      }
    }
  );
 ...
 */
export const journeyProgressResultsSelector = (journeyId, filter = new Map({ journey: journeyId })) =>
  createSelector(
    [modelsByFilterSelector('journeyProgress', filter), countSelector('journeyProgress', filter)],
    (journeyProgresses, count) =>
      new Map({
        journeyProgresses,
        count
      })
 ...
 */
const getPipelinesResults = (state) => (pipelines) =>
  pipelines.map((pipeline) => aggregationResultsSelector(pipeline)(state) || new Map());
 ...
 * @return {Immutable.List}
 */
export const visualisationResultsSelector = (visualisationId, filter) =>
  createSelector([identity, modelsSchemaIdSelector('visualisation', visualisationId)], (state, visualisation) => {
    switch (visualisation.get('type')) {
      case JOURNEY_PROGRESS:
...
export const visualisationAllAggregationsHaveResultSelector = (visualisationId) =>
  createSelector([identity], (state) => {
    const series = visualisationPipelinesSelector(visualisationId)(state);
    return series.reduce(
      (acc1, pipelines) =>
        acc1 && pipelines.reduce((acc2, pipeline) => acc2 && aggregationHasResultSelector(pipeline)(state), true),
      true
    );
  });
