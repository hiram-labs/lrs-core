 ...
 * Actions
 */
export const alert = ({ schema, id, message, alertType = DANGER, options }) => ({
  type: ALERT,
  alertType,
  schema,
  id,
...
const alertFailureStart = ({ schema, id, message, alertType, options, uuid = uniqueId() }) => ({
  type: ALERT_START,
  schema,
  id,
  message,
...
export const deleteAlert = ({ key }) => ({
  type: DELETE_ALERT,
  key
});

const deleteAlertByUuid = ({ uuid }) => ({
...
export const getAlertsSelector = createSelector([alertsSelector], (alertState) =>
  alertState.filter(
    (alertItem) =>
      get(alertItem, ['options', 'status']) !== 404 && get(alertItem, ['options', 'name']) !== 'ValidationError'
  )
);
