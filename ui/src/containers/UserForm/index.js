...
import { hasScopeSelector, loggedInUserId as loggedInUserIdSelector } from 'ui/redux/modules/auth';
...
const changeModelAttr = (updateModel, model, attr) => (value) => updateModel({ path: [attr], value });
...
const renderVerified = (model) => {
  const verifiedId = uuid();
  const isVerified = model.get('verified');

  return (
    <div className="form-group">
      <label htmlFor={verifiedId} className="control-label">
        Verified:
      </label>
      <span id={verifiedId}>
        <VerifiedIcon isVerified={isVerified} className={`icon ${isVerified ? 'ion-checkmark' : 'ion-close'}`} />
      </span>
    </div>
  );
};
...
const renderPasswordChanges = (model, onCheck, changePasswordChecked) => {
  ...
  return (
    <div className="form-group">
      ...
      <div id={passwordId}>
        {model.get('verified') ? (
          <Checkbox key="change-password" checked={changePasswordChecked} label="Change password" onChange={onCheck} />
        ) : (
...
const renderPasswordConfirmation = (passwordConfirmation, setPasswordConfirmation, passwordGroupClasses) => {
  ...
  return (
    <div className={passwordGroupClasses}>
      <label htmlFor={confirmationId} className="control-label">
        Confirm Password
      </label>
      <input
        id={confirmationId}
...
const changeModelPassword = (saveModel, model, password, setPassword, setPasswordConfirmation) => () => {
...
const UserForm = ({
  ...
  const passwordErrors = validatePassword(password, passwordConfirmation, ownerOrganisationSettings).concat(
    password === '' ? serverErrors : new List()
  );
  const hasPasswordErrors = !passwordErrors.isEmpty();
  const canChangePassword = changePasswordChecked || hasPasswordErrors;
  const isAuthorisedToChangePassword = isSiteAdmin || model.get('_id') === loggedInUserId;
  ...
  return (
    <div className="row">
      <div className="col-md-12">
        ...
        {isAuthorisedToChangePassword &&
          renderPasswordChanges(
            model,
            onPasswordCheckboxChange(updateModel, model, setChangePasswordChecked),
            canChangePassword
          )}
        ...
        {passwordInputsVisible && (
          <div className="form-group">
            ...
            <button
              ...
              onClick={changeModelPassword(saveModel, model, password, setPassword, setPasswordConfirmation)}>
