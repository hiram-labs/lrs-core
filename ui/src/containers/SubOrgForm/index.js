...
import { uploadLogo, IN_PROGRESS, SUCCESS, FAILED } from 'ui/redux/modules/logo';
...
class SubOrgForm extends Component {
  ...
  renderButtonContent = () => {
    const exportProgress = this.props.uploadState.getIn([schema, this.props.model.get('_id'), 'uploadState']);
        ...
        <div
          ...
          {errorMessages.has('settings.LOCKOUT_ATTEMPS') && (
            <span className="help-block">
              <ValidationList errors={errorMessages.get('settings.LOCKOUT_ATTEMPS')} />
            </span>
          )}
        </div>
        <div
          ...
          {errorMessages.has('settings.LOCKOUT_SECONDS') && (
            <span className="help-block">
              <ValidationList errors={errorMessages.get('settings.LOCKOUT_SECONDS')} />
            </span>
          )}
        </div>
      </div>
    ...
    return (
      <div>
        <div
          ...
          <label htmlFor={passwordTotalId}>Check against how many previous passwords?</label>
          <input
            id={passwordTotalId}
            className="form-control"
            placeholder="How many previous consecutive passwords are checked?"
            value={settings.get('PASSWORD_HISTORY_TOTAL')}
            onChange={this.onChangeSettingsAttr.bind(null, 'PASSWORD_HISTORY_TOTAL')} />

          {errorMessages.has('settings.PASSWORD_HISTORY_TOTAL') && (
            <span className="help-block">
              <ValidationList errors={errorMessages.get('settings.PASSWORD_HISTORY_TOTAL')} />
            </span>
          )}
        </div>
      </div>
  ...
  renderLogoSetting = () => {
    ...
    return (
      <form onSubmit={this.handleSubmit} encType="multipart/form-data">
        <label className="btn btn-primary btn-inverse btn-default" htmlFor={logoId}>
          <input
            id={logoId}
            name="logo"
            type="file"
    ...
    return (
      <div
        ...
        {errorMessages.has('settings.PASSWORD_MIN_LENGTH') && (
          <span className="help-block">
            <ValidationList errors={errorMessages.get('settings.PASSWORD_MIN_LENGTH')} />
          </span>
        )}
      </div>
    );
    ...
    return (
      <div
        ...
        <input
          ...
          onChange={this.onChangeSettingsAttr.bind(null, 'PASSWORD_CUSTOM_REGEX')} />

        {errorMessages.has('settings.PASSWORD_CUSTOM_REGEX') && (
          <span className="help-block">
            <ValidationList errors={errorMessages.get('settings.PASSWORD_CUSTOM_REGEX')} />
          </span>
        )}
      </div>
    );
    ...
    return (
      <div
        ...
        <input
          ...
          onChange={this.onChangeSettingsAttr.bind(null, 'PASSWORD_CUSTOM_MESSAGE')} />

        {errorMessages.has('settings.PASSWORD_CUSTOM_MESSAGE') && (
          <span className="help-block">
            <ValidationList errors={errorMessages.get('settings.PASSWORD_CUSTOM_MESSAGE')} />
          </span>
        )}
      </div>
    );
        ...
        <div className="col-md-12">
          <fieldset>
            ...
            <div className="form-group">
              ...
              <br />
              <a onClick={this.handleSubmit} className="btn btn-primary btn-inverse btn-sm">
          ...
          <fieldset>
              ...
              <div>
                <div className="form-group">
                  <Checkbox
                    ...
                    onChange={this.onChangeBooleanSetting.bind(null, 'PASSWORD_REQUIRE_ALPHA')}
                ...
                <div className="form-group">
                  <Checkbox
                    ...
                    onChange={this.onChangeBooleanSetting.bind(null, 'PASSWORD_REQUIRE_NUMBER')}
            ...
            <div className="form-group">
              <Checkbox
                ...
                onChange={this.onChangeBooleanSetting.bind(null, 'PASSWORD_USE_CUSTOM_REGEX')}
            ...
            {settings.get('PASSWORD_USE_CUSTOM_REGEX') && (
              <div>
                <p>
                  Using custom password requirements allows you to create your own validation rules using a Regular
                  Expression.
                </p>
            ...
            <div className="form-group">
              <Checkbox
                ...
                onChange={this.onChangeBooleanSetting.bind(null, 'LOCKOUT_ENABLED')}
                checked={settings.get('LOCKOUT_ENABLED')} />
            </div>

            {settings.get('LOCKOUT_ENABLED') && this.renderLockoutSetting(errorMessages, settings)}
            ...
            <div className="form-group">
              <Checkbox
                ...
                onChange={this.onChangeBooleanSetting.bind(null, 'PASSWORD_HISTORY_CHECK')}
                checked={settings.get('PASSWORD_HISTORY_CHECK')} />
            </div>

            {settings.get('PASSWORD_HISTORY_CHECK') && this.renderPasswordHistoryCheck(errorMessages, settings)}
          </fieldset>
          ...
          {this.props.isSiteAdmin && usageStats && (
            <div className="form-group">
              <p>
                Usage: {usageStats.get('OWN_COUNT').toLocaleString('en')} statements and approximately{' '}
                {Math.round((usageStats.get('OWN_ESTIMATED_BYTES') / 1000000) * 100) / 100} MB
              </p>

              {usageStats.get('HAS_CHILDREN') && (
                <p>
                  Total Usage: {usageStats.get('TOTAL_COUNT').toLocaleString('en')} statements and approximately{' '}
                  {Math.round((usageStats.get('TOTAL_ESTIMATED_BYTES') / 1000000) * 100) / 100} MB
                </p>
              )}
            </div>
          )}
