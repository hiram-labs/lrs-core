...
const utilHandlers = withHandlers({
  updateSelectedSharable:
    ({ parentModel, model, updateModel }) =>
    (pathValues) => {
      const newModel = pathValues.reduce((acc, { path, value }) => acc.set(path, value), model);
      const selectedIndex = parentModel.get('shareable').findIndex((item) => item.get('_id') === model.get('_id'));
...
const handlers = withHandlers({
  ...
  onChangeTimezone:
    ({ model, organisationModel, updateSelectedSharable }) =>
    (value) => {
...
const getErrors = (parentModel, model) => {
  ...
  return relevantMessages.toMap().mapKeys((key, value) => value.get('key'));
};
...
const ModelFormComponent = ({
  ...
  const filterId = uuid();
  const urlId = uuid();
  const visibilityId = uuid();
  const validDomainsId = uuid();
  const filterModeId = uuid();
  const filterRequiredId = uuid();
  const filterJwtSecretId = uuid();
  const filterTzId = uuid();
  ...
  return (
    <div>
      <div className="form-group">
        ...
        <input className="form-control" id={titleId} value={model.get('title', '')} onChange={handleTitleChange} />
      </div>
      <div className="form-group">
      ...
      <div className="form-group">
        <button className="btn btn-primary" onClick={copyToClipBoard(urlId)}>
          Copy link
        </button>
      ...
      <div className="form-group" style={{ marginTop: 25, marginBottom: 25 }}>
        <label htmlFor={visibilityId}>Where can this be viewed?</label>
        <RadioGroup
          id={visibilityId}
          name="visibility"
          ...
          <RadioButton label="Only where I choose" value={VALID_DOMAINS} />
        </RadioGroup>
      </div>

      {model.get('visibility') === VALID_DOMAINS ? (
        <div className="form-group" style={{ marginBottom: 25 }}>
          <label htmlFor={validDomainsId}>What are the valid domains?</label>
          <div>
            <DebounceInput
              id={validDomainsId}
              className="form-control"
          ...
          <ContextHelp className={'help-block'}>
            ...
            matching any hostname this dashboard will be embedded into
          </ContextHelp>
        </div>
      ) : null}

      <div className="form-group" style={{ marginBottom: 25 }}>
        <label htmlFor={filterModeId}>URL filter mode</label>
        <RadioGroup id={filterModeId} name="filterMode" value={filterMode} onChange={handleFilterModeChange}>
      ...
      {filterMode === JWT_SECURED ? (
        <div
          ...
          <label htmlFor={filterJwtSecretId}>JWT Secret (HS256)</label>
          <ContextHelp className={'help-block'}>
      ...
      {filterMode === JWT_SECURED ? (
        <div className="form-group" style={{ marginBottom: 25 }}>
          <label htmlFor={filterJwtSecretId}>URL filter required</label>
          <ContextHelp className={'help-block'}>
      ...
      <div className="form-group">
        ...
        <ContextHelp className={'help-block'}>
          ...
          preview of the result, updating as you construct your filter
          <br />
          <br />
          If a URL filter is also used, it will be applied on top of this filter
        </ContextHelp>
...
const ModelForm = compose(
  connect((state) => ({ organisationModel: activeOrgSelector(state) })),
  utilHandlers,
  handlers
)(ModelFormComponent);

const deleteButton = ({ parentModel }) =>
  compose(
...
const openLinkButton = ({ parentModel }) =>
  compose(
    withHandlers({
      openLink:
        ({ id }) =>
...
const dashboardSharingHandlers = withHandlers({
  addShareable:
    ({ updateModel, model }) =>
    () => {
      const newShareable = model.get('shareable', new List()).push(
        new Map({
          title: 'Shareable',
          createdAt: new Date(),
          timezone: null
...
const DashboardSharingComponent = ({ model, updateModel, addShareable }) => (
  <div>
    <div className={'clearfix'} style={{ marginBottom: 20 }}>
      <button className="btn btn-primary pull-right" onClick={addShareable}>
        New shareable link
      </button>
    </div>
    <ModelList
