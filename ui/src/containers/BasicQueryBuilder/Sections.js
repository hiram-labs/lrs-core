...
export default class Sections extends Component {
  ...
  getCriteriaForSection = (section, used = false) => {
    ...
    const out = this.props.criteria.filter((value, key) => {
      ...
      const matchChildGenerators = section
        .get('childGenerators', new List())
        .filter((generator) => criteriaPath.slice(0, generator.get('path').size).equals(generator.get('path')))
        .count();
  ...
  getCriteriaForWholeSection = (section, used = false) =>
    this.getCriteriaForSection(section, used).count() + this.getCriteriaForChildren(section);
