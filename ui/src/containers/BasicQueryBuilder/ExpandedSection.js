...
export const queryBuilderCacheFilterGenerator = ({ section }) => {
  ...
  if (section.get('childGenerators', false)) {
    ...
    const orQueryList = map(childGenerators.toJS(), (item) => {
      ...
      return merge({}, ...map(keyPath, (item2, index) => ({ [`path.${index}`]: { $eq: item2 } })));
    });

    const maxItem = maxBy(orQueryList, (item) => size(item));
    const maxSize = size(maxItem);

    query = { $and: [{ $or: orQueryList }, { path: { $size: maxSize + 1 } }] };
  } else if (section.has('keyPath')) {
    const andQuery = map(section.get('keyPath', new List()).toJS(), (item, index) => ({
      [`path.${index}`]: { $eq: item }
    }));

    query = { $and: [{ $and: andQuery }, { path: { $size: size(andQuery) + 1 } }] };
  }

  return { queryBuilderCacheFilter: query };
};

const withQueryBuilderCacheProps = withProps(queryBuilderCacheFilterGenerator);
...
class ExpandedSection extends Component {
  ...
  renderActorDiscreteCriteria = () => (
    <ActorDiscreteCriteria
      section={this.props.section}
      criteria={this.props.sectionCriteria}
      onCriteriaChange={this.props.onCriteriaChange}
      onAddCriterion={this.props.onAddCriterion}
  ...
  renderCriteria = () => {
    ...
    switch (ops) {
      case operators.CONTINUOUS:
        return this.renderContinuousCriteria();
      ...
      case operators.BOOLEAN:
        return this.renderBooleanCriteria();
      case operators.STRING_MATCHES:
        return this.renderStringMatchesCriteria();
      case operators.OR_DISCRETE:
        return this.renderActorDiscreteCriteria();
      default:
        return this.renderDiscreteCriteria();
    }
  };
  ...
  render = () => {
    ...
    return (
      <ExpandedSectionWrapper>
        ...
        {hasCriteria && <div style={{ marginTop: 10 }}>{this.renderCriteria()}</div>}
        {children.size > 0 && (
          <ExpandedSectionsContainer
            timezone={this.props.timezone}
            orgTimezone={this.props.orgTimezone}
...
export default compose(
  ...
  connect((state, { queryBuilderCacheFilter, section }) => {
    const availableSection = getAvailableSection(section, queryBuilderCacheFilter)(state);
