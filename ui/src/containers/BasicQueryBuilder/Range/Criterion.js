...
export class Criterion extends Component {
  ...
  canDeleteCriterion = () => this.props.onDeleteCriterion !== undefined;
  ...
  getCriterionSubQuery = (operator, value) => {
    switch (operator) {
      case '>':
        return new Map({ $gt: this.getValueQuery(value) });
      case '>=':
        return new Map({ $gte: this.getValueQuery(value) });
      case '<=':
        return new Map({ $lte: this.getValueQuery(value) });
      case '==':
        return new Map({ $eq: this.getValueQuery(value) });
      case '!=':
        return new Map({ $ne: this.getValueQuery(value) });
      default:
        return new Map({ $lt: this.getValueQuery(value) });
    }
  };
  ...
  render = () => {
    ...
    return (
      <CriterionWrapper>
        ...
        {canDeleteCriterion && (
          <CriterionButton className={'btn btn-default btn-xs'} onClick={this.props.onDeleteCriterion}>
