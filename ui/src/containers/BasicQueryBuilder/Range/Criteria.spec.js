...
test('It should generate initial Criterion', () => {
  ...
  expect(testCriteria.renderCriterion.mock.calls[0][0].get('statement.result.score.scaled')).toEqual(
    new Map({ $gt: '' })
  );
  expect(testCriteria.renderCriterion.mock.calls[0][1]).toEqual(
    new Map({
      criterionLabel: 'A',
