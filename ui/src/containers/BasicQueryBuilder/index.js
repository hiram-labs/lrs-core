...
const enhance = compose(
  ...
  shouldUpdate(
    (prev, next) =>
      !(prev.timezone === next.timezone && prev.orgTimezone === next.orgTimezone && prev.query.equals(next.query))
  )
);
...
const BasicQueryBuilder = ({ componentPath, timezone, orgTimezone, defaults, query, onQueryChange }) => {
  ...
  return (
    <QueryBuilderSections
      ...
      criteria={criteria}
      defaults={defaults}
      onCriteriaChange={(newCriteria) => onQueryChange(changeCriteria(newCriteria))}
      onAddCriterion={(criterion, section) => onQueryChange(addCriterionFromSection(query, criterion, section))}
