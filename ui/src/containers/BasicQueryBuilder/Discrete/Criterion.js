...
class Criterion extends Component {
  ...
  shouldComponentUpdate = ({ section, criterion, filter }) =>
    !(this.props.section.equals(section) && this.props.criterion.equals(criterion) && this.props.filter.equals(filter));

  /**
   * @returns {string}
   */
  getQueryKey = () => this.props.section.get('getQueryKey', '');

  getSearchStringToFilter = () => this.props.section.get('searchStringToFilter');

  getOptionQuery = (option) => this.props.section.get('getModelQuery')(option);

  getOptionDisplay = (option) => this.props.section.get('getModelDisplay')(option);

  getOptionIdentifier = (option) => this.props.section.get('getModelIdent')(option);
  ...
  onChangeOperator = (operator) => this.onChangeCriterion(operator, this.getValues());
