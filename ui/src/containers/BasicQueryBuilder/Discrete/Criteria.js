...
export default class Criteria extends Component {
  ...
  shouldComponentUpdate = (nextProps) =>
    !(this.props.section.equals(nextProps.section) && this.props.criteria.equals(nextProps.criteria));
  ...
  renderCriteria = () => <div>{this.props.criteria.map(this.renderCriterion.bind(this)).valueSeq()}</div>;

  renderEmptyCriteria = () => <TempCriterion section={this.props.section} onCriterionChange={this.createCriterion} />;

  render = () => <div>{this.props.criteria.count() > 0 ? this.renderCriteria() : this.renderEmptyCriteria()}</div>;
}
