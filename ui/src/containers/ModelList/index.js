...
const enhance = compose(
  ...
  connect((state, { schema, models }) => {
    const metadata = models.map((model) => getMetadataSelector({ schema, id: model.get('_id') })(state));
  ...
  withHandlers({
    fetchMore:
      ({ schema, filter, sort, fetchMore }) =>
      () =>
        fetchMore({ schema, filter, sort })
  }),
  withProps(({ models, model }) => ({
    modelsWithModel:
      !model || !model.get('_id') || models.has(model.get('_id'))
        ? models
  ...
  lifecycle({
    componentDidUpdate() {
      const { setMetadata, isExpandedOnce, setExpandedOnce, model } = this.props;
  ...
  if (modelsWithModel.size > 0) {
    return (
      <div>
        ...
        {isLoading ? <Spinner /> : renderLoadMoreButton({ isLoading, hasMore, fetchMore })}
      </div>
    );
  } else if (isLoading) {
    return <Spinner />;
