...
class CollapsibleNav extends Component {
  ...
  render = () => {
    ...
    const { children, label, icon, routeName, routeParams, activeRoute, manuallyExpanded } = this.props;
    ...
    return (
      <li>
        ...
        <VelocityComponent animation={animationProps} duration={350} complete={this.animationComplete(isExpanded)}>
          <SubNav className={'nav animated fadeIn'} ref={this.saveBodyRef} style={{ overflow: 'hidden' }}>
            {children}
          </SubNav>
        </VelocityComponent>
      </li>
