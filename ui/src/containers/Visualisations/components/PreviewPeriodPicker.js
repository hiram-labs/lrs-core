 ...
 */
const PreviewPeriodPicker = ({ visualisationId, previewPeriod, onChange }) => {
  const formId = `visualisation-previewPeriod-${visualisationId}`;
  return (
    <select id={formId} className="form-control" value={previewPeriod} onChange={(e) => onChange(e.target.value)}>
