 ...
 */
const BenchmarkingEnabledSwitch = ({ visualisationId, benchmarkingEnabled, onChange }) => {
  const formId = `visualisation-enable-benchmarking-${visualisationId}`;
  return (
    <div className="form-group">
      <label htmlFor={formId}>Enable Benchmarking</label>

      <div id={formId}>
        <Switch checked={benchmarkingEnabled} onChange={onChange} />
      </div>
    </div>
  );
};
