 ...
 */
const StackedSwitch = ({ visualisationId, stacked, onChange }) => {
  const formId = `visualisation-stacked-${visualisationId}`;
  return (
    <div className="form-group">
      <label htmlFor={formId}>Stacked/Grouped</label>

      <div id={formId}>
        <Switch checked={!stacked} onChange={(checked) => onChange(!checked)} />
      </div>
    </div>
  );
};
