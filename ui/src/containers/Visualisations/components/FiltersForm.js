 ...
 */
const MultipleFilters = ({ visualisationId, filters, timezone, orgTimezone, onChange }) => {
  const [tabIndex, setTabIndex] = useState(0);

  return (
    <Tabs index={tabIndex} onChange={setTabIndex}>
      {filters.map((filter, i) => (
        <Tab key={i} label={filter.get('label') || `Series ${i + 1}`}>
          <FilterForm
 ...
 * @param {(filters: immutable.List) => void} props.onChange
 */
const FiltersFormContents = ({ visualisationId, filters, canEditLabel, timezone, orgTimezone, onChange }) => {
  if (filters.size === 0) {
    return <NoFilters />;
  }
 ...
 * @param {(filters: immutable.List) => void} props.onChange
 */
const FiltersForm = ({ visualisationId, filters, canEditLabel, timezone, orgTimezone, onChange }) => {
  const formId = `visualisation-filters-form-${visualisationId}`;

  return (
    <div className="form-group">
      <label htmlFor={formId}>Build your query</label>
