...
import { compose, withHandlers, shouldUpdate, setPropTypes, defaultProps } from 'recompose';
...
const enhance = compose(
  ...
  withHandlers({
    onDelete:
      ({ onChange, widgets }) =>
      (selectedModel) => {
        const newWidgets = widgets.filter((model) => model.get('_id') !== selectedModel.get('_id')).toList();
  ...
  dimensions(),
  shouldUpdate((props, nextProps) => !nextProps.widgets.equals(props.widgets))
);
