...
const ModelAutoComplete = ({
  ...
  searchFilterToString = (searchFilter2) =>
    (searchFilter2 && searchFilter2.first() && searchFilter2.first().get('$regex')) || '',
  displayCount,
  fields,
  placeholder,
  defaultValues,
    ...
    <AutoComplete2
      renderInput={({ hasFocus, onFocus }) => (
        <ModelInput
          ...
          parseOption={(option) =>
            option ? (parseOptionString && parseOptionString(option)) || parseOption(option) : ''
          }
...
export default compose(withSelectedOption, withSearchFilter)(ModelAutoComplete);
