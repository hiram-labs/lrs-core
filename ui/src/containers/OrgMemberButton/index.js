...
const enhance = compose(
  ...
  withHandlers({
    handleJoin:
      ({ updateModel, userOrgs, model, authUser }) =>
      (e) => {
        updateModel({
          schema: 'user',
          id: authUser.get('_id'),
    ...
    handleLeave:
      ...
      (e) => {
        updateModel({
          ...
          id: authUser.get('_id'),
          path: ['organisations'],
          value: userOrgs.remove(userOrgs.findIndex((userOrg) => model.get('_id') === userOrg))
        });
        e.stopPropagation();
      }
  })
...
const OrgMemberButton = ({ handleJoin, handleLeave, model, userOrgs }) => {
  ...
  return hasOrg ? (
    ...
    <button title="Join organisation" className="btn-sm btn btn-inverse" onClick={handleJoin} style={{ width: '33px' }}>
