...
class Dashboard extends Component {
  ...
  createPopulatedWidget = (widgetIndex) => {
    ...
    const newModel = model.get('widgets', new List()).push(
      new Map({
        ...
        <header id="topbar">
          <div className="heading heading-light">
            <span className="pull-right open_panel_btn">
              <a onClick={this.onClickAddWidget} className="btn btn-primary btn-sm">
        ...
        <div className="row">
          {this.props.getMetadata('isSharing') && (
            <div className="col-md-12">
              <DashboardSharing shareable={model.get('shareable', new List())} id={model.get('_id')} />
            </div>
          )}
...
export default compose(
  ...
  lifecycle({
    componentDidUpdate(previousProps) {
      if (
        this.props.model.get('widgets') &&
        this.props.model.get('widgets').size &&
        previousProps.size &&
        this.props.model.get('widgets').size > previousProps.model.get('widgets').size &&
        window
      ) {
  ...
  mapProps((original) =>
    _.pick(original, ['model', 'updateModel', 'saveModel', 'setMetadata', 'getMetadata', 'backToDashboard'])
  )
)(Dashboard);
