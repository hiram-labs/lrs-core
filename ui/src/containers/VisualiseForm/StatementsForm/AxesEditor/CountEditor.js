...
import { getTypeOpts, UNIQUENESS_OPS, VALUE_OPTS, UNIQUENESS_VALUE_OPTS } from 'ui/utils/visualisations/localOptions';
...
export default class CountEditor extends Component {
  ...
  includes = (ys) => (xs) => xs.indexOf(ys) !== -1;

  isUniquenessOp = (op) => this.includes(op)(UNIQUENESS_OPS);

  getOperatorOptions = () => getTypeOpts(this.props.type).filter(this.includes(this.state.operatorInput));

  changeOperatorInput = (newOperator) => this.setState({ operatorInput: newOperator });
