...
const TemplateCardList = ({ model, saveModel }) => (
  <CardList id="new-visualisation-templates">
    <TemplateLearningExperienceType model={model} saveModel={saveModel} />

    <TemplateLast7DaysStatementsCard model={model} saveModel={saveModel} />

    <TemplateActivityOverTimeCard model={model} saveModel={saveModel} />

    <TemplateMostPopularVerbsCard model={model} saveModel={saveModel} />

    <TemplateMostPopularActivitiesCard model={model} saveModel={saveModel} />

    <TemplateMostActivePeopleCard model={model} saveModel={saveModel} />

    <TemplateWeekdaysActivityCard model={model} saveModel={saveModel} />

    <TemplateStreamInteractionsVsEngagementCard model={model} saveModel={saveModel} />

    <TemplateStreamCommentCountCard model={model} saveModel={saveModel} />

    <TemplateStreamLearnerInteractionsByDateAndVerbCard model={model} saveModel={saveModel} />

    <TemplateStreamUserEngagementLeaderboardCard model={model} saveModel={saveModel} />

    <TemplateStreamProportionOfSocialInteractionsCard model={model} saveModel={saveModel} />

    <TemplateStreamActivitiesWithMostCommentsCard model={model} saveModel={saveModel} />
  </CardList>
);
