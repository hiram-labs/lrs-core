...
export const generateTableData = (results, labels) => {
  ...
  const result = seriesMap.reduce(
    (reduction, series, seriesKey) =>
      series.reduce(
        (axesReduction, axes2, axesKey) =>
          axes2.reduce((seriesReduction, item) => {
            ...
            return seriesReduction.setIn(dataKeyName, item).setIn(modelKeyName, item.get('model'));
          }, axesReduction),
        reduction
      ),
    new OrderedMap()
...
const countSubColumns = (labels, tableData) =>
  labels.reduce(
    (acc1, label) =>
      tableData.reduce((acc2, row) => Math.max(acc2, row.getIn(['rowData', label], new Map()).size), acc1),
    1
  );
...
const renderStatsTableRows = ({ stats, subColumnsCount }) =>
      ...
      {stats
        ...
        .map((_, sIndex) =>
          [...Array(subColumnsCount).keys()].map((i) => (
            <th key={`${sIndex}-${i}`}>{formatNumber(stats.getIn([sIndex, i, key]))}</th>
          ))
        )
        .flat()}
    </tr>
...
const SourceResult = ({ getFormattedResults, results, labels, visualisation }) => {
    ...
    <div style={{ height: '100%' }}>
      <div style={{ height: mightBeLimited ? 'calc(100% - 20px)' : '100%' }}>
        <ScrollableTable>
            ...
            {moreThanOneSeries(tableData) && (
              <tr>
                ...
                {tLabels.toArray().map((tLabel, i) => (
                  <th key={i} colSpan={subColumnsCount}>
                    {tLabel}
                  </th>
                ))}
              </tr>
              ...
              {tLabels
                ...
                .map((_, i) =>
                  [...Array(subColumnsCount).keys()].map((j) => (
                    <th key={`${i}-${j}`}>{getValueAxisLabel(j, visualisation)}</th>
                  ))
                )
                .flat()}
            </tr>

            {showStatsAtTop && renderStatsTableRows({ stats, subColumnsCount })}
          </thead>
          ...
          {statsAtBottom && <tfoot>{renderStatsTableRows({ stats, subColumnsCount })}</tfoot>}
        </ScrollableTable>
      </div>
