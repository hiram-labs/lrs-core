...
export class DashboardCard extends Component {
    ...
    return (
      <div>
        ...
        >
          ...
          <a
            onClick={() => {
              this.openModal('isDuplicateOpen');
            }}
            title="Duplicate"
          >
            Duplicate
          ...
          <a
            onClick={() => {
              this.openModal('isPrivacyOpen');
            }}
            title="Privacy toggle"
          >
            {isPublic ? <span>Set to Private</span> : <span>Set to Public</span>}
          </a>

          <a
            onClick={() => {
              this.openModal('isDeleteOpen');
            }}
            title="Delete button"
          >
            Delete
  ...
  render = () => {
    const { model, handleDashboard, index } = this.props;
    ...
    return (
      <div className={styles.dashboardCard}>
        <div className={styles.menuWrapper}>{this.renderMenu()}</div>
        <div key={index} onClick={handleDashboard} className={classNames(styles.card)}>
          <img className={styles.cardIcon} src={iconType()} alt={title} />
          ...
          <h4 className={styles.cardTitle} onClick={handleDashboard}>
            {title}
          </h4>
...
export default compose(
  ...
  connect(
    ...
    (dispatch) => ({
      copyDashboard: ({ dashboard, visualisations, organisationId, userId }) =>
        dispatch({
          type: COPY_DASHBOARD,
          dispatch,
          dashboard,
  ...
  withHandlers({
    doCopyDashboard:
      ({ model, state, organisationId, copyDashboard, userId }) =>
      () =>
        copyDashboard({
          dashboard: model,
