...
import { COUNTER, PIE, TEMPLATE_LAST_7_DAYS_STATEMENTS, TEMPLATE_STREAM_COMMENT_COUNT } from 'lib/constants/visualise';
...
class Widget extends Component {
  ...
  isCounter = (type) =>
    type === COUNTER || type === TEMPLATE_LAST_7_DAYS_STATEMENTS || type === TEMPLATE_STREAM_COMMENT_COUNT;
  ...
  renderMenu = () => {
    ...
    const shouldShowGoVisualisation = model.has('visualisation') && visualisation.size > 0;
    ...
    return (
      ...
      >
        {shouldShowTableModeToggle && (
          <CloseButton onClick={this.toggleSourceView} title="Table mode">
        ...
        {shouldShowDonutModeToggle && (
          <CloseButton onClick={this.toggleDonutView} title="Donut mode">
            <WidgetDropdownIcon className={'ion ion-edit grey'} />
            {this.getDonutView()}
          </CloseButton>
        )}
        ...
        {this.props.editable && (
          <a onClick={this.openDeleteModal} title="Delete Widget">
        ...
        <WidgetContent>
          ...
          </WidgetHeading>
          {
            <WidgetBody className={'panel-body'}>
              {model.has('visualisation') && <VisualisationViewer id={model.get('visualisation')} />}
            </WidgetBody>
          }
          {isModalOpen && (
