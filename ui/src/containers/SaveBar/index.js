...
import { Container, Label, StyledProgressBar } from 'ui/containers/SaveBar/styled';

export const savingSelector = () =>
  createSelector(
    (
      state // models
    ) =>
      state.models
        .map((model) =>
          model.filter(
            (item) =>
              item && item.getIn && (!!item.getIn(['remoteCache', 'requestState']) || !!item.getIn(['deleteState']))
          )
        )
        .toList()
        .flatMap((model) =>
          model.toList().flatMap(
            (item) =>
  ...
  return (
    <Container className={'save-bar'}>
      <StyledProgressBar
        ...
        value={100}
      />
      <Label savingStageClassName={savingStageClassName}>{getLabel(saving)}</Label>
      <SaveBarErrors />
    </Container>
  );
};
