...
const withCacheKeys = compose(
  ...
  withProps(({ models }) => ({
    models: models
      .mapKeys((k, model) => model.get('searchString'))
      .map((model) => model.set('optionKey', model.get('searchString')))
  }))
);

const withLocalKeys = compose(
  withProps(({ localOptions, searchString = '' }) => ({
    localOptions: localOptions.filter((option) => includes(searchString.toLowerCase())(option.get('searchString')))
  }))
);
