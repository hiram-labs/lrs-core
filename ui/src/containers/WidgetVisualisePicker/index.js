...
class WidgetVisualisePicker extends Component {
  ...
  onClickVisualisation = (visualisation) => {
    if (!visualisation) {
      return this.props.onChangeVisualisation(null, this.props.model.get('title') || 'No visualisation');
    }
    const out = this.props.onChangeVisualisation(visualisation.get('_id'), visualisation.get('description', ''));
    return out;
  };
  ...
  renderTitle = () => {
    ...
    return (
      <div className="form-group" style={{ width: '100%' }}>
        <label htmlFor={htmlFor} className="clearfix">
          Widget Title
        </label>
        <input
          id={htmlFor}
...
export default compose(withProps({ schema }), withModels)(WidgetVisualisePicker);
