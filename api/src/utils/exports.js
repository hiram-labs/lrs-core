...
export const streamToCsv = (headers, stream) =>
...
export const exportCSV = ({ authInfo, pipelines }) =>
  new Promise((resolve, reject) => {
    const headers = pipelines.reduce((result, pipeline) => {
      ...
      const visibleHeaders = projectedHeaders.filter((projectedHeader) => lastProjection[projectedHeader] !== 0);
