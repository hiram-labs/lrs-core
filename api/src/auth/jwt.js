...
const payloadDefaults = ({ provider = 'native', scopes = [], extensions = {}, organisation = null, ...others }) => ({
  provider,
  scopes,
  extensions,
  organisation,
 ...
 */
const createJWT = (
  { userId, provider, scopes, filter, tokenType, tokenId, shareableId = null, extensions, organisation = null },
  opts = {
    expiresIn: '1h'
  }
) =>
...
const createUserTokenPayload = (user, provider) =>
  payloadDefaults({
...
const createUserJWT = (user, provider) =>
  createJWT(createUserTokenPayload(user, provider), { expiresIn: JWT_ACCESS_TOKEN_EXPIRATION });

const createUserRefreshTokenPayload = (user, provider) =>
  payloadDefaults({
    userId: String(user._id),
    provider,
...
const createUserRefreshJWT = (user, provider) =>
  createJWT(createUserRefreshTokenPayload(user, provider), { expiresIn: JWT_REFRESH_TOKEN_EXPIRATION });
...
const createOrgTokenPayload = async (user, orgId, provider) => {
  ...
  const scopes = [...roleScopes, ...getUserScopes(user)];
...
const createOrgJWT = async (user, organisationId, provider) => {
  ...
  return createJWT(orgTokenPayload, { expiresIn: JWT_ACCESS_TOKEN_EXPIRATION });
};

const createOrgRefreshTokenPayload = (user, organisationId, provider) =>
  payloadDefaults({
    userId: String(user._id),
    provider,
...
const createOrgRefreshJWT = (user, organisationId, provider) =>
  createJWT(createOrgRefreshTokenPayload(user, organisationId, provider), { expiresIn: JWT_REFRESH_TOKEN_EXPIRATION });
...
const createDashboardJWT = async (dashboard, shareableId, provider) => {
  ...
  return createJWT(dashboardPayload, { expiresIn: JWT_ACCESS_TOKEN_EXPIRATION });
};
