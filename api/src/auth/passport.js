...
import { createOrgTokenPayload, createUserTokenPayload, createDashboardTokenPayload } from 'api/auth/jwt';
...
const createPayloadFromPayload = (payload) => {
  ...
  return Promise.props(dependencies).then(async ({ user, dashboard }) => {
    ...
    switch (payload.tokenType) {
      case 'organisation': {
        const expectedToken = await createOrgTokenPayload(user, tokenId, provider);
...
passport.use(
  ...
  new BasicStrategy((clientId, clientSecret, done) => {
    Client.findOne({ 'api.basic_key': clientId }).then((client) => {
...
if (process.env.GOOGLE_ENABLED && process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  passport.use(
    new GoogleStrategy(
      {
      ...
      (accessToken, refreshToken, profile, done) => {
        const userEmail = find(profile.emails, (email) => email.verified === true);

        User.findOne({ email: userEmail.value }).then((user) => {
          if (!user) {
          ...
          user.save().then((savedUser) => done(null, savedUser)).catch((err) => done(err, null));
...
passport.use(
  ...
  new CustomStrategy((req, done) => {
    ...
    if (grantType === undefined || clientId === undefined || clientSecret === undefined) {
