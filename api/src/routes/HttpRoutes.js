...
import { omit, findIndex, get, pick } from 'lodash';
 ...
 * AUTHORIZATION
 */
router.post(routes.AUTH_RESETPASSWORD_REQUEST, AuthController.resetPasswordRequest);
...
router.post(routes.AUTH_JWT_REFRESH, AuthController.jwtRefresh);
...
router.get(routes.OAUTH2_FAILED, (request, response) => {
  response.send('Authorization failed');
});

router.post(routes.OAUTH2_TOKEN, AuthController.issueOAuth2AccessToken);
...
if (process.env.GOOGLE_ENABLED) {
  router.get(routes.AUTH_JWT_GOOGLE, passport.authenticate('google', GOOGLE_AUTH_OPTIONS));

  router.get(routes.AUTH_JWT_GOOGLE_CALLBACK, (request, response, next) => {
    passport.authenticate('google', DEFAULT_PASSPORT_OPTIONS, (error, user, info) => {
...
router.get(routes.AUTH_JWT_SUCCESS, passport.authenticate('jwt', DEFAULT_PASSPORT_OPTIONS), AuthController.success);
 ...
 * UPLOADS
 */
router.post(routes.UPLOADLOGO, passport.authenticate('jwt', DEFAULT_PASSPORT_OPTIONS), UploadController.uploadLogo);
 ...
 * EXPORTS
 */
router.get(routes.EXPORT, passport.authenticate('jwt', DEFAULT_PASSPORT_OPTIONS), ExportController.downloadExport);
...
restify.serve(router, Stream, RESTIFY_DEFAULTS);
restify.serve(router, Export, RESTIFY_DEFAULTS);
restify.serve(router, Download, RESTIFY_DEFAULTS);
restify.serve(router, Query, RESTIFY_DEFAULTS);
restify.serve(router, ImportCsv, RESTIFY_DEFAULTS);
  ...
  preUpdate: (req, _, next) => {
    ...
    if (!scopes.includes(SITE_ADMIN)) {
      ...
      const isUpdatingItself =
        ['user', 'organisation'].includes(tokenType) && req.body._id === getUserIdFromAuthInfo(authInfo).toString();
...
restify.serve(router, Client, RESTIFY_DEFAULTS);
restify.serve(router, Visualisation, RESTIFY_DEFAULTS);
restify.serve(router, Dashboard, RESTIFY_DEFAULTS);
restify.serve(router, LRS, RESTIFY_DEFAULTS);
...
restify.serve(router, StatementForwarding, RESTIFY_DEFAULTS);
restify.serve(router, QueryBuilderCache, RESTIFY_DEFAULTS);
restify.serve(router, QueryBuilderCacheValue, RESTIFY_DEFAULTS);
restify.serve(router, Role, RESTIFY_DEFAULTS);
restify.serve(router, PersonaAttribute, {
  ...RESTIFY_DEFAULTS,
  preDelete: async (req, res, next) => next()
});
restify.serve(router, PersonasImport, RESTIFY_DEFAULTS);
restify.serve(router, PersonasImportTemplate, RESTIFY_DEFAULTS);
restify.serve(router, SiteSettings, RESTIFY_DEFAULTS);
...
const generateModelRoutes = (model) => {
  ...
  const authentication = (req, res, next) =>
    passport.authenticate(['jwt', 'clientBasic'], DEFAULT_PASSPORT_OPTIONS, (err, user) => {
      if (err || !user) {
