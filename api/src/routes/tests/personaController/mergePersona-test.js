...
const assertError = (expectedConstructor, promise) =>
  promise.then(() => { assert(false, 'Expected an error to be thrown'); }).catch((err) => {
    if (err instanceof expectedConstructor) return;
...
describe('PersonaController.mergePersona', () => {
  ...
  const mergePersona = async ({ mergePersonaFromId, mergePersonaToId, expectedCode }) =>
  ...
  const assertFailedMerge = async ({ mergePersonaFromId, mergePersonaToId, expectedCode }) => {
    await mergePersona({
      mergePersonaFromId,
      mergePersonaToId,
      expectedCode
