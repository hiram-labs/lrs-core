...
describe('personaController getPersonaOnly', () => {
  ...
  it('should get a single persona', async () => {
    ...
    const result = await apiApp.get(`/v2/persona/${persona.id}`).set('Authorization', `Bearer ${token}`).expect(200);
  ...
  it('should get multiple personas', async () => {
    ...
    const result = await apiApp.get('/v2/persona').set('Authorization', `Bearer ${token}`).expect(200);
