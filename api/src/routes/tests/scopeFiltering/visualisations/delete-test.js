...
import { EDIT_ALL_VISUALISATIONS, EDIT_PUBLIC_VISUALISATIONS } from 'lib/constants/orgScopes';
...
describe('API HTTP DELETE visualisations route scope filtering', () => {
  ...
  const assertVisualisation = async ({ isPublic, isOwner, bearerToken, basicClient, expectedStatus = 204 }) => {
    const visualisation = await createVisualisation(isPublic, isOwner ? TEST_USER_ID : new ObjectId());
  ...
  const assertPublicVisualisation = async ({ bearerToken, basicClient, expectedStatus = 204 }) =>
    assertVisualisation({
      isPublic: true,
      isOwner: false,
      bearerToken,
  ...
  const assertMyBearerPublicVisualisation = async ({ token, expectedStatus = 204 }) =>
    assertVisualisation({
      isPublic: true,
      isOwner: true,
      bearerToken: token,
  ...
  const assertPrivateVisualisation = async ({ bearerToken, basicClient, expectedStatus = 204 }) =>
    assertVisualisation({
      isPublic: false,
      isOwner: false,
      bearerToken,
  ...
  const assertMyBearerPrivateVisualisation = async ({ token, expectedStatus = 204 }) =>
    assertVisualisation({
      isPublic: false,
      isOwner: true,
      bearerToken: token,
