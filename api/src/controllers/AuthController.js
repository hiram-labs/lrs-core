  ...
  passport.authenticate('userBasic', { session: false }, (err, data) => {
    ...
    if (!data.success || !user) {
      ...
      if (user) {
        ...
        if (!user.authLockoutExpiry && user.authFailedAttempts >= user.ownerOrganisationSettings.LOCKOUT_ATTEMPS) {
          user.authLockoutExpiry = new Date();
        }
        user.authLastAttempt = new Date();
        return user.save().then(() => authFailure(data.reason));
      }
    ...
    return user.save().then(() =>
      Promise.all([createUserJWT(user), createUserRefreshJWT(user)])
        .then(([accessToken, refreshToken]) =>
          res
            .cookie(`refresh_token_user_${user._id}`, refreshToken, buildRefreshCookieOption(req.protocol))
...
const includes = (organisations, orgId) =>
  organisations.filter((organisationId) => String(organisationId) === orgId).length !== 0;
...
const jwtOrganisation = (req, res) => {
  ...
  if (includes(user.organisations, organisationId)) {
    Promise.all([
      ...
      .then(([orgAccessToken, orgRefreshToken]) =>
        res
          .cookie(
