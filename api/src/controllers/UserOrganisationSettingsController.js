...
const createOrganisationSetting = catchErrors(async (req, res) => {
  ...
  if (alreadyExists) {
    throw new ClientError(
      `Duplicated: The user already has the organisationSettings for the organisation (${req.params.organisationId})`
    );
  }
  ...
  if (req.params.organisationId !== req.body.organisation) {
    throw new ClientError(
      `Invalid: organisationId in URL path (${req.params.organisationId}) and organisation in body (${req.body.organisation}) are not matched.`
    );
  }

  user.organisationSettings.push(req.body);
  const updatedUser = await user.save();
  const insertedOrganisationSetting = updatedUser.organisationSettings.find(
    (s) => s.organisation.toString() === req.params.organisationId
  );
...
const deleteOrganisationSetting = catchErrors(async (req, res) => {
  ...
  user.organisationSettings = user.organisationSettings.filter(
    (s) => s.organisation.toString() !== req.params.organisationId
  );
  await user.save();
  res.status(200).send();
