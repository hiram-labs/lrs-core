...
import { get, has, head, tail, isString, filter, isObject, find } from 'lodash';
...
const uploadPersonas = catchErrors(async (req, res) => {
  ...
  const {
    file,
    fields: { id }
  } = await getFileAndFieldsFromRequest(req);
...
const uploadJsonPersona = catchErrors(async (req, res) => {
  ...
  const attributes = filter(
    inputAttributes,
    (inputAttribute) => has(inputAttribute, 'key') && has(inputAttribute, 'value')
  );
