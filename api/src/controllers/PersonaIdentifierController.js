...
import { CursorDirection } from '@hiram-labs/lrs-persona-service/dist/service/constants';
import Locked from '@hiram-labs/lrs-persona-service/dist/errors/Locked';
...
const personaIdentifierConnection = catchErrors(async (req, res) => {
  ...
  const { persona, ...inFilter } = parsedFilter;
...
const addPersonaIdentifier = catchErrors(async (req, res) => {
  ...
  asignIdentifierToStatements({ organisation, toIdentifierId: identifier.id }).catch(handleError);
